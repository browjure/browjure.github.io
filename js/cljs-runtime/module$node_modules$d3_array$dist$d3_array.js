...
matrix.length))return[];for(var i=-1,m=min$jscomp$0(matrix,length$jscomp$0),transpose=Array(m);++i<m;)for(var j=-1,n,row=transpose[i]=Array(n);++j<n;)row[j]=matrix[j][i];return transpose}function length$jscomp$0(d){return d.length}function set$jscomp$0(values){return values instanceof Set?values:new Set(values)}function superset(values,other){values=values[Symbol.iterator]();const set=new Set;for(const o of other){if(set.has(o))continue;let done;for(;{value:other,done}=values.next();){if(done)return!1;
set.add(other);if(Object.is(o,other))break}}return!0}var ascendingBisect=bisector(ascending);const bisectRight=ascendingBisect.right;ascendingBisect=ascendingBisect.left;const bisectCenter=bisector(number).center;class Adder{constructor(){this._partials=new Float64Array(32);this._n=0}add(x){const p=this._partials;let i=0;for(let j=0;j<this._n&&32>j;j++){const y=p[j],hi=x+y;(x=Math.abs(x)<Math.abs(y)?x-(hi-y):y-(hi-x))&&(p[i++]=x);x=hi}p[i]=x;this._n=i+1;return this}valueOf(){const p=this._partials;
let n=this._n;let y,hi=0;if(0<n){for(hi=p[--n];0<n;){var x=hi;y=p[--n];hi=x+y;if(x=y-(hi-x))break}0<n&&(0>x&&0>p[n-1]||0<x&&0<p[n-1])&&(y=2*x,x=hi+y,y==x-hi&&(hi=x))}return hi}}class InternMap extends Map{constructor(entries,key$jscomp$0=keyof){super();Object.defineProperties(this,{_intern:{value:new Map},_key:{value:key$jscomp$0}});if(null!=entries)for(const [key,value]of entries)this.set(key,value)}get(key){return super.get(intern_get(this,key))}has(key){return super.has(intern_get(this,key))}set(key,
value){return super.set(intern_set(this,key),value)}delete(key){return super.delete(intern_delete(this,key))}}class InternSet extends Set{constructor(values,key=keyof){super();Object.defineProperties(this,{_intern:{value:new Map},_key:{value:key}});if(null!=values)for(const value of values)this.add(value)}has(value){return super.has(intern_get(this,value))}add(value){return super.add(intern_set(this,value))}delete(value){return super.delete(intern_delete(this,value))}}var slice=Array.prototype.slice,
e10=Math.sqrt(50),e5=Math.sqrt(10),e2=Math.sqrt(2),shuffle=shuffler(Math.random);exports.Adder=Adder;exports.InternMap=InternMap;exports.InternSet=InternSet;exports.ascending=ascending;exports.bin=bin;exports.bisect=bisectRight;exports.bisectCenter=bisectCenter;exports.bisectLeft=ascendingBisect;exports.bisectRight=bisectRight;exports.bisector=bisector;exports.count=count$jscomp$0;exports.cross=function(...values){const reduce="function"===typeof values[values.length-1]&&reducer$jscomp$0(values.pop());
values=values.map(arrayify);const lengths=values.map(length$1),j$jscomp$0=values.length-1,index=Array(j$jscomp$0+1).fill(0),product=[];if(0>j$jscomp$0||lengths.some(empty))return product;for(;;){product.push(index.map((j,i)=>values[i][j]));let i$jscomp$0=j$jscomp$0;for(;++index[i$jscomp$0]===lengths[i$jscomp$0];){if(0===i$jscomp$0)return reduce?product.map(reduce):product;index[i$jscomp$0--]=0}}};exports.cumsum=function(values,valueof){var sum=0,index=0;return Float64Array.from(values,void 0===valueof?
v=>sum+=+v||0:v=>sum+=+valueof(v,index++,values)||0)};exports.descending=function(a,b){return b<a?-1:b>a?1:b>=a?0:NaN};exports.deviation=deviation;exports.difference=function(values,...others){values=new Set(values);for(const other of others)for(const value of other)values.delete(value);return values};exports.disjoint=function(values,other){other=other[Symbol.iterator]();const set=new Set;for(const v of values){if(set.has(v))return!1;let done;for(;({value:values,done}=other.next())&&!done;){if(Object.is(v,
values))return!1;set.add(values)}}return!0};exports.every=function(values,test){if("function"!==typeof test)throw new TypeError("test is not a function");let index=-1;for(const value of values)if(!test(value,++index,values))return!1;return!0};exports.extent=extent;exports.fcumsum=function(values,valueof){const adder=new Adder;let index=-1;return Float64Array.from(values,void 0===valueof?v=>adder.add(+v||0):v=>adder.add(+valueof(v,++index,values)||0))};exports.filter=function(values,test){if("function"!==
typeof test)throw new TypeError("test is not a function");const array=[];let index=-1;for(const value of values)test(value,++index,values)&&array.push(value);return array};exports.fsum=function(values,valueof){const adder=new Adder;if(void 0===valueof)for(var value$jscomp$0 of values)(value$jscomp$0=+value$jscomp$0)&&adder.add(value$jscomp$0);else{value$jscomp$0=-1;for(let value of values)(value=+valueof(value,++value$jscomp$0,values))&&adder.add(value)}return+adder};exports.greatest=function(values,
compare=ascending){let max,defined=!1;if(1===compare.length){let maxValue;for(const element of values)if(values=compare(element),defined?0<ascending(values,maxValue):0===ascending(values,values))max=element,maxValue=values,defined=!0}else for(const value of values)if(defined?0<compare(value,max):0===compare(value,value))max=value,defined=!0;return max};exports.greatestIndex=function(values,compare=ascending){if(1===compare.length)return maxIndex(values,compare);let maxValue,max=-1,index=-1;for(const value of values)if(++index,
0>max?0===compare(value,value):0<compare(value,maxValue))maxValue=value,max=index;return max};exports.group=group$jscomp$0;exports.groupSort=function(values,reduce,key$jscomp$0){return(1===reduce.length?sort(rollup(values,reduce,key$jscomp$0),([ak,av],[bk,bv])=>ascending(av,bv)||ascending(ak,bk)):sort(group$jscomp$0(values,key$jscomp$0),([ak,av],[bk,bv])=>reduce(av,bv)||ascending(ak,bk))).map(([key])=>key)};exports.groups=function(values,...keys){return nest(values,Array.from,identity,keys)};exports.histogram=
bin;exports.index=function(values,...keys){return nest(values,identity,unique,keys)};exports.indexes=function(values,...keys){return nest(values,Array.from,unique,keys)};exports.intersection=function(values,...others){values=new Set(values);others=others.map(set$jscomp$0);a:for(const value of values)for(const other of others)if(!other.has(value)){values.delete(value);continue a}return values};exports.least=function(values,compare=ascending){let min,defined=!1;if(1===compare.length){let minValue;for(const element of values)if(values=
compare(element),defined?0>ascending(values,minValue):0===ascending(values,values))min=element,minValue=values,defined=!0}else for(const value of values)if(defined?0>compare(value,min):0===compare(value,value))min=value,defined=!0;return min};exports.leastIndex=leastIndex;exports.map=function(values,mapper){if("function"!==typeof values[Symbol.iterator])throw new TypeError("values is not iterable");if("function"!==typeof mapper)throw new TypeError("mapper is not a function");return Array.from(values,
(value,index)=>mapper(value,index,values))};exports.max=max$jscomp$0;exports.maxIndex=maxIndex;exports.mean=function(values,valueof){let count=0,sum=0;if(void 0===valueof)for(var value$jscomp$0 of values)null!=value$jscomp$0&&(value$jscomp$0=+value$jscomp$0)>=value$jscomp$0&&(++count,sum+=value$jscomp$0);else{value$jscomp$0=-1;for(let value of values)null!=(value=valueof(value,++value$jscomp$0,values))&&(value=+value)>=value&&(++count,sum+=value)}if(count)return sum/count};exports.median=function(values,
valueof){return quantile(values,.5,valueof)};exports.merge=function(arrays){return Array.from(flatten(arrays))};exports.min=min$jscomp$0;exports.minIndex=minIndex;exports.nice=nice;exports.pairs=function(values,pairof=pair){const pairs=[];let previous,first=!1;for(const value of values)first&&pairs.push(pairof(previous,value)),previous=value,first=!0;return pairs};exports.permute=permute;exports.quantile=quantile;exports.quantileSorted=function(values,p,valueof=number){if(n=values.length){if(0>=(p=
