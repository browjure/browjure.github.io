...
map.style._loaded&&map.getSource(id)&&map.removeSource(id)})}}},[map,id]);(context=map&&map.style&&map.getSource(id))?updateSource(context,props,propsRef.current):context=createSource(map,id,props);propsRef.current=props;return context&&React.Children.map(props.children,function(child){return child&&(0,React.cloneElement)(child,{source:id})})||null}module=require("module$node_modules$$babel$runtime$helpers$interopRequireWildcard");global=require("module$node_modules$$babel$runtime$helpers$interopRequireDefault");
