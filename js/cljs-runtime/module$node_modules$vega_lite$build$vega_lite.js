...
3);lowerD=SHORT_DAYS.indexOf(lowerD);if(-1!==lowerD)return lowerD;throw Error(invalidTimeUnit("day",d));}}function dateTimeParts(d,normalize){const parts=[];normalize&&void 0!==d.day&&1<keys(d).length&&(warn("Dropping day from datetime ".concat(stringify(d)," as day cannot be combined with other units.")),d=duplicate(d),delete d.day);void 0!==d.year?parts.push(d.year):parts.push(2012);if(void 0!==d.month){var month=normalize?normalizeMonth(d.month):d.month;parts.push(month)}else if(void 0!==d.quarter){if(normalize)if(month=
d.quarter,isNumeric(month)&&(month=+month),isNumber(month))4<month&&warn(invalidTimeUnit("quarter",month)),--month;else throw Error(invalidTimeUnit("quarter",month));else month=d.quarter;parts.push(isNumber(month)?3*month:month+"*3")}else parts.push(0);void 0!==d.date?parts.push(d.date):void 0!==d.day?(normalize=normalize?normalizeDay(d.day):d.day,parts.push(isNumber(normalize)?normalize+1:normalize+"+1")):parts.push(1);for(const timeUnit of["hours","minutes","seconds","milliseconds"])normalize=d[timeUnit],
parts.push("undefined"===typeof normalize?0:normalize);return parts}function dateTimeToExpr(d){const string=dateTimeParts(d,!0).join(", ");return d.utc?"utc(".concat(string,")"):"datetime(".concat(string,")")}function dateTimeToTimestamp(d){const parts=dateTimeParts(d,!0);return d.utc?+new Date(Date.UTC(...parts)):+new Date(...parts)}function getTimeUnitParts(timeUnit){const parts=[];for(const part of TIMEUNIT_PARTS)containsTimeUnit(timeUnit,part)&&parts.push(part);return parts}function containsTimeUnit(fullTimeUnit,
timeUnit){const index=fullTimeUnit.indexOf(timeUnit);return 0>index||0<index&&"seconds"===timeUnit&&"i"===fullTimeUnit.charAt(index-1)||fullTimeUnit.length>index+3&&"day"===timeUnit&&"o"===fullTimeUnit.charAt(index+3)||0<index&&"year"===timeUnit&&"f"===fullTimeUnit.charAt(index-1)?!1:!0}function timeUnitSpecifierExpression(timeUnit){if(timeUnit)return timeUnit=getTimeUnitParts(timeUnit),"timeUnitSpecifier(".concat(fastJsonStableStringify(timeUnit),", ").concat(fastJsonStableStringify(VEGALITE_TIMEFORMAT),
")")}function normalizeTimeUnit(timeUnit){if(timeUnit){var params;isString(timeUnit)?params={unit:timeUnit}:isObject(timeUnit)&&(params=Object.assign({},timeUnit,timeUnit.unit?{unit:timeUnit.unit}:{}));params.unit.startsWith("utc")&&(params.utc=!0,params.unit=params.unit.substr(3));return params}}function timeUnitToString(tu){var $jscomp$destructuring$var13=normalizeTimeUnit(tu);tu=Object.assign({},$jscomp$destructuring$var13);$jscomp$destructuring$var13=$jscomp$destructuring$var13.utc;const rest=
(delete tu.utc,tu);return rest.unit?($jscomp$destructuring$var13?"utc":"")+keys(rest).map(p=>varName("".concat("unit"===p?"":"_".concat(p,"_")).concat(rest[p]))).join(""):($jscomp$destructuring$var13?"utc":"")+"timeunit"+keys(rest).map(p=>varName("_".concat(p,"_").concat(rest[p]))).join("")}function isSignalRef(o){return o&&!!o.signal}function isVgRangeStep(range){return!!range.step}function isDataRefDomain(domain){return isArray(domain)?!1:"field"in domain&&"data"in domain}function isFieldEqualPredicate(predicate){return predicate&&
...
(isArray(predicate.range)&&2===predicate.range.length||isSignalRef(predicate.range))?!0:!1}function isFieldOneOfPredicate(predicate){return predicate&&!!predicate.field&&(isArray(predicate.oneOf)||isArray(predicate.in))}function isFieldPredicate(predicate){return isFieldOneOfPredicate(predicate)||isFieldEqualPredicate(predicate)||isFieldRangePredicate(predicate)||isFieldLTPredicate(predicate)||isFieldGTPredicate(predicate)||isFieldLTEPredicate(predicate)||isFieldGTEPredicate(predicate)}function predicateValueExpr(v,
timeUnit){return valueExpr(v,{timeUnit,wrapTime:!0})}function predicateValuesExpr(vals,timeUnit){return vals.map(v=>predicateValueExpr(v,timeUnit))}function fieldFilterExpression(predicate,useInRange){useInRange=void 0===useInRange?!0:useInRange;var _normalizeTimeUnit,{field}=predicate;const timeUnit=null===(_normalizeTimeUnit=normalizeTimeUnit(predicate.timeUnit))||void 0===_normalizeTimeUnit?void 0:_normalizeTimeUnit.unit;if(timeUnit){{({end:_normalizeTimeUnit}={end:!1});field=accessPathWithDatum(field);
const utc=timeUnit.startsWith("utc")?"utc":"";const dateExpr={};for(JSCompiler_temp of TIMEUNIT_PARTS)if(containsTimeUnit(timeUnit,JSCompiler_temp)){var lastTimeUnit=JSCompiler_temp;lastTimeUnit="quarter"===lastTimeUnit?"(".concat(utc,"quarter(").concat(field,")-1)"):"".concat(utc).concat(lastTimeUnit,"(").concat(field,")");dateExpr[JSCompiler_temp]=lastTimeUnit;lastTimeUnit=JSCompiler_temp}_normalizeTimeUnit&&(dateExpr[lastTimeUnit]+="+1");var JSCompiler_temp=dateExpr;_normalizeTimeUnit=dateTimeParts(JSCompiler_temp,
!1).join(", ");JSCompiler_temp=JSCompiler_temp.utc?"utc(".concat(_normalizeTimeUnit,")"):"datetime(".concat(_normalizeTimeUnit,")")}JSCompiler_temp="time("+JSCompiler_temp+")"}else JSCompiler_temp=vgField(predicate,{expr:"datum"});if(isFieldEqualPredicate(predicate))return JSCompiler_temp+"\x3d\x3d\x3d"+predicateValueExpr(predicate.equal,timeUnit);if(isFieldLTPredicate(predicate))return useInRange=predicate.lt,"".concat(JSCompiler_temp,"\x3c").concat(predicateValueExpr(useInRange,timeUnit));if(isFieldGTPredicate(predicate))return useInRange=
predicate.gt,"".concat(JSCompiler_temp,"\x3e").concat(predicateValueExpr(useInRange,timeUnit));if(isFieldLTEPredicate(predicate))return useInRange=predicate.lte,"".concat(JSCompiler_temp,"\x3c\x3d").concat(predicateValueExpr(useInRange,timeUnit));if(isFieldGTEPredicate(predicate))return useInRange=predicate.gte,"".concat(JSCompiler_temp,"\x3e\x3d").concat(predicateValueExpr(useInRange,timeUnit));if(isFieldOneOfPredicate(predicate))return"indexof([".concat(predicateValuesExpr(predicate.oneOf,timeUnit).join(","),
"], ").concat(JSCompiler_temp,") !\x3d\x3d -1");if(predicate&&predicate.field&&void 0!==predicate.valid)return fieldValidPredicate(JSCompiler_temp,predicate.valid);if(isFieldRangePredicate(predicate)){({range:_normalizeTimeUnit}=predicate);predicate=isSignalRef(_normalizeTimeUnit)?{signal:"".concat(_normalizeTimeUnit.signal,"[0]")}:_normalizeTimeUnit[0];_normalizeTimeUnit=isSignalRef(_normalizeTimeUnit)?{signal:"".concat(_normalizeTimeUnit.signal,"[1]")}:_normalizeTimeUnit[1];if(null!==predicate&&
...
"true"}throw Error("Invalid field predicate: ".concat(JSON.stringify(predicate)));}function fieldValidPredicate(fieldExpr,valid){return void 0===valid||valid?"isValid(".concat(fieldExpr,") \x26\x26 isFinite(+").concat(fieldExpr,")"):"!isValid(".concat(fieldExpr,") || !isFinite(+").concat(fieldExpr,")")}function normalizePredicate(f){if(isFieldPredicate(f)&&f.timeUnit){var _normalizeTimeUnit2;return Object.assign({},f,{timeUnit:null===(_normalizeTimeUnit2=normalizeTimeUnit(f.timeUnit))||void 0===_normalizeTimeUnit2?
void 0:_normalizeTimeUnit2.unit})}return f}function getFullName(type){if(type)switch(type=type.toLowerCase(),type){case "q":case "quantitative":return"quantitative";case "t":case "temporal":return"temporal";case "o":case "ordinal":return"ordinal";case "n":case "nominal":return"nominal";case "geojson":return"geojson"}}function isSelectionDomain(domain){return null===domain||void 0===domain?void 0:domain.selection}function scaleTypeSupportProperty(scaleType,propName){switch(propName){case "type":case "domain":case "reverse":case "range":return!0;
case "scheme":case "interpolate":return!contains(["point","band","identity"],scaleType);case "bins":return!contains(["point","band","identity","ordinal"],scaleType);case "round":return scaleType in CONTINUOUS_TO_CONTINUOUS_INDEX||"band"===scaleType||"point"===scaleType;case "padding":case "rangeMin":case "rangeMax":return scaleType in CONTINUOUS_TO_CONTINUOUS_INDEX||contains(["point","band"],scaleType);case "paddingOuter":case "align":return contains(["point","band"],scaleType);case "paddingInner":return"band"===
scaleType;case "domainMax":case "domainMid":case "domainMin":case "clamp":return scaleType in CONTINUOUS_TO_CONTINUOUS_INDEX;case "nice":return scaleType in CONTINUOUS_TO_CONTINUOUS_INDEX||"quantize"===scaleType||"threshold"===scaleType;case "exponent":return"pow"===scaleType;case "base":return"log"===scaleType;case "constant":return"symlog"===scaleType;case "zero":return scaleType in CONTINUOUS_DOMAIN_INDEX&&!contains(["log","time","utc","threshold","quantile"],scaleType)}}function channelScalePropertyIncompatability(channel,
propName){switch(propName){case "interpolate":case "scheme":case "domainMid":if(!isColorChannel(channel))return'Cannot use the scale property "'.concat(channel,'" with non-color channel.')}}function channelSupportScaleType(channel,scaleType){if(!SCALE_CHANNEL_INDEX[channel])return!1;switch(channel){case "x":case "y":case "theta":case "radius":return scaleType in CONTINUOUS_TO_CONTINUOUS_INDEX||contains(["band","point"],scaleType);case "size":case "strokeWidth":case "opacity":case "fillOpacity":case "strokeOpacity":case "angle":return scaleType in
CONTINUOUS_TO_CONTINUOUS_INDEX||scaleType in CONTINUOUS_TO_DISCRETE_INDEX||contains(["band","point","ordinal"],scaleType);case "color":case "fill":case "stroke":return"band"!==scaleType;case "strokeDash":return"ordinal"===scaleType||scaleType in CONTINUOUS_TO_DISCRETE_INDEX;case "shape":return"ordinal"===scaleType}}function isExprRef(o){return o&&!!o.expr}function replaceExprRefInIndex(index){const props=keys(index||{}),newIndex={};for(const prop of props)newIndex[prop]=signalRefOrValue(index[prop]);
return newIndex}function isPathMark(m){return contains(["line","area","trail"],m)}function isMarkDef(mark){return mark.type}function midPointRefWithPositionInvalidTest(params){const {channel,channelDef,markDef,scale,config}=params;params=midPoint(params);return isFieldDef(channelDef)&&!isCountingAggregateOp(channelDef.aggregate)&&scale&&scale.get("type")in CONTINUOUS_TO_CONTINUOUS_INDEX&&!1===scale.get("zero")?wrapPositionInvalidTest({fieldDef:channelDef,channel,markDef,ref:params,config}):params}
function wrapPositionInvalidTest($jscomp$destructuring$var17){var {fieldDef,channel,markDef,ref,config}=$jscomp$destructuring$var17;if(isPathMark(markDef.type)||null===getMarkPropOrConfig("invalid",markDef,config))return ref;{$jscomp$destructuring$var17=fieldInvalidPredicate(fieldDef,!0);const zeroValueRef="y"===getMainRangeChannel(channel)?{field:{group:"height"}}:{value:0};$jscomp$destructuring$var17=Object.assign({},{test:$jscomp$destructuring$var17},zeroValueRef)}return[$jscomp$destructuring$var17,
ref]}function fieldInvalidPredicate(field,invalid){invalid=void 0===invalid?!0:invalid;return fieldValidPredicate(isString(field)?field:vgField(field,{expr:"datum"}),!invalid)}function valueRefForFieldOrDatumDef(fieldDef,scaleName,opt,encode){const ref={};scaleName&&(ref.scale=scaleName);isDatumDef(fieldDef)?({datum:fieldDef}=fieldDef,isDateTime(fieldDef)?ref.signal=dateTimeToExpr(fieldDef):isSignalRef(fieldDef)?ref.signal=fieldDef.signal:isExprRef(fieldDef)?ref.signal=fieldDef.expr:ref.value=fieldDef):
ref.field=vgField(fieldDef,opt);if(encode){const {offset,band}=encode;offset&&(ref.offset=offset);band&&(ref.band=band)}return ref}function interpolatedSignalRef($jscomp$destructuring$var18){var {scaleName,fieldOrDatumDef,fieldOrDatumDef2,offset,startSuffix,band=.5}=$jscomp$destructuring$var18,expr=0<band&&1>band?"datum":void 0;$jscomp$destructuring$var18=vgField(fieldOrDatumDef,{expr,suffix:startSuffix});const end=void 0!==fieldOrDatumDef2?vgField(fieldOrDatumDef2,{expr}):vgField(fieldOrDatumDef,
{suffix:"end",expr});expr={};0===band||1===band?(expr.scale=scaleName,expr.field=0===band?$jscomp$destructuring$var18:end):($jscomp$destructuring$var18="".concat(band," * ").concat($jscomp$destructuring$var18," + ").concat(1-band," * ").concat(end),expr.signal='scale("'.concat(scaleName,'", ').concat($jscomp$destructuring$var18,")"));offset&&(expr.offset=offset);return expr}function midPoint($jscomp$destructuring$var19){var {channel,channelDef,channel2Def,markDef,config,scaleName,scale,stack,offset,
defaultRef,band}=$jscomp$destructuring$var19;if(channelDef){if(isFieldOrDatumDef(channelDef)){var _ref,_band2;if(isTypedFieldDef(channelDef)){var _band;band=null!==(_band=band)&&void 0!==_band?_band:getBand({channel,fieldDef:channelDef,fieldDef2:channel2Def,markDef,stack,config,isMidPoint:!0});const {bin,timeUnit,type}=channelDef;if(isBinning(bin)||band&&timeUnit&&"temporal"===type)return stack&&stack.impute?valueRefForFieldOrDatumDef(channelDef,scaleName,{binSuffix:"mid"},{offset}):band?interpolatedSignalRef({scaleName,
fieldOrDatumDef:channelDef,band,offset}):valueRefForFieldOrDatumDef(channelDef,scaleName,binRequiresRange(channelDef,channel)?{binSuffix:"range"}:{},{offset});if(isBinned(bin)){if(isFieldDef(channel2Def))return interpolatedSignalRef({scaleName,fieldOrDatumDef:channelDef,fieldOrDatumDef2:channel2Def,band,offset});warn(channelRequiredForBinned("x"===channel?"x2":"y2"))}}$jscomp$destructuring$var19=null===scale||void 0===scale?void 0:scale.get("type");return valueRefForFieldOrDatumDef(channelDef,scaleName,
$jscomp$destructuring$var19 in DISCRETE_DOMAIN_INDEX?{binSuffix:"range"}:{},{offset,band:"band"===$jscomp$destructuring$var19?null!==(_ref=null!==(_band2=band)&&void 0!==_band2?_band2:channelDef.band)&&void 0!==_ref?_ref:.5:void 0})}if(isValueDef(channelDef))return _ref=offset?{offset}:{},Object.assign({},widthHeightValueOrSignalRef(channel,channelDef.value),_ref)}"function"===typeof defaultRef&&(defaultRef=defaultRef());return defaultRef?Object.assign({},defaultRef,offset?{offset}:{}):defaultRef}
function widthHeightValueOrSignalRef(channel,value){return contains(["x","x2"],channel)&&"width"===value?{field:{group:"width"}}:contains(["y","y2"],channel)&&"height"===value?{field:{group:"height"}}:signalOrValueRef(value)}function isCustomFormatType(formatType){return formatType&&"number"!==formatType&&"time"!==formatType}function customFormatExpr(formatType,field,format){return"".concat(formatType,"(").concat(field).concat(format?", ".concat(JSON.stringify(format)):"",")")}function formatSignalRef($jscomp$destructuring$var20){var {fieldOrDatumDef,
format,formatType,expr,normalizeStack,config}=$jscomp$destructuring$var20;if(isCustomFormatType(formatType))return formatCustomType({fieldOrDatumDef,format,formatType,expr,config});$jscomp$destructuring$var20=fieldToFormat(fieldOrDatumDef,expr,normalizeStack);if(isFieldOrDatumDefForTimeFormat(fieldOrDatumDef)){var _normalizeTimeUnit,_fieldOrDatumDef$scal;var timeUnit=isFieldDef(fieldOrDatumDef)?null===(_normalizeTimeUnit=normalizeTimeUnit(fieldOrDatumDef.timeUnit))||void 0===_normalizeTimeUnit?void 0:
_normalizeTimeUnit.unit:void 0,format$jscomp$0=format,rawTimeFormat=config.timeFormat;_normalizeTimeUnit=isScaleFieldDef(fieldOrDatumDef)&&(null===(_fieldOrDatumDef$scal=fieldOrDatumDef.scale)||void 0===_fieldOrDatumDef$scal?void 0:_fieldOrDatumDef$scal.type)===ScaleType.UTC;!timeUnit||format$jscomp$0?(format$jscomp$0=isString(format$jscomp$0)?format$jscomp$0:rawTimeFormat,$jscomp$destructuring$var20="".concat(_normalizeTimeUnit?"utc":"time","Format(").concat($jscomp$destructuring$var20,", '").concat(format$jscomp$0,
"')")):timeUnit?(_fieldOrDatumDef$scal=timeUnitSpecifierExpression(timeUnit),timeUnit=_normalizeTimeUnit||timeUnit.startsWith("utc"),$jscomp$destructuring$var20="".concat(timeUnit?"utc":"time","Format(").concat($jscomp$destructuring$var20,", ").concat(_fieldOrDatumDef$scal,")")):$jscomp$destructuring$var20=void 0;return $jscomp$destructuring$var20?{signal:$jscomp$destructuring$var20}:void 0}format=numberFormat(channelDefType(fieldOrDatumDef),format,config);return isFieldDef(fieldOrDatumDef)&&isBinning(fieldOrDatumDef.bin)?
(_fieldOrDatumDef$scal=vgField(fieldOrDatumDef,{expr,binSuffix:"end"}),{signal:binFormatExpression($jscomp$destructuring$var20,_fieldOrDatumDef$scal,format,formatType,config)}):format||"quantitative"===channelDefType(fieldOrDatumDef)?{signal:"".concat(formatExpr($jscomp$destructuring$var20,format))}:{signal:"isValid(".concat($jscomp$destructuring$var20,") ? ").concat($jscomp$destructuring$var20,' : ""+').concat($jscomp$destructuring$var20)}}function fieldToFormat(fieldOrDatumDef,expr,normalizeStack){if(isFieldDef(fieldOrDatumDef))return normalizeStack?
"".concat(vgField(fieldOrDatumDef,{expr,suffix:"end"}),"-").concat(vgField(fieldOrDatumDef,{expr,suffix:"start"})):vgField(fieldOrDatumDef,{expr});({datum:fieldOrDatumDef}=fieldOrDatumDef);fieldOrDatumDef=isDateTime(fieldOrDatumDef)?dateTimeToExpr(fieldOrDatumDef):"".concat(JSON.stringify(fieldOrDatumDef));return fieldOrDatumDef}function formatCustomType($jscomp$destructuring$var21){var {fieldOrDatumDef,format,formatType,expr,normalizeStack,config,field}=$jscomp$destructuring$var21,_field;field=null!==
...
fieldOrDatumDef=isFieldDef(fieldOrDatumDef)?null===(_normalizeTimeUnit2=normalizeTimeUnit(fieldOrDatumDef.timeUnit))||void 0===_normalizeTimeUnit2?void 0:_normalizeTimeUnit2.unit:void 0;format=format?format:fieldOrDatumDef?{signal:timeUnitSpecifierExpression(fieldOrDatumDef)}:omitTimeFormatConfig?void 0:config.timeFormat;return format}return numberFormat(type,format,config)}}function guideFormatType(formatType,fieldOrDatumDef,scaleType){if(formatType&&(isSignalRef(formatType)||"number"===formatType||
"time"===formatType))return formatType;if(isFieldOrDatumDefForTimeFormat(fieldOrDatumDef)&&"time"!==scaleType&&"utc"!==scaleType)return"time"}function numberFormat(type,specifiedFormat,config){if(isString(specifiedFormat))return specifiedFormat;if("quantitative"===type)return config.numberFormat}function formatExpr(field,format){return"format(".concat(field,', "').concat(format||"",'")')}function binNumberFormatExpr(field,format,formatType,config){var _ref;return isCustomFormatType(formatType)?customFormatExpr(formatType,
field,format):formatExpr(field,null!==(_ref=isString(format)?format:void 0)&&void 0!==_ref?_ref:config.numberFormat)}function binFormatExpression(startField,endField,format,formatType,config){const start=binNumberFormatExpr(startField,format,formatType,config);endField=binNumberFormatExpr(endField,format,formatType,config);return"".concat(fieldValidPredicate(startField,!1),' ? "null" : ').concat(start,' + "').concat(" – ",'" + ').concat(endField)}function isSortField(sort){return!!sort&&("count"===
sort.op||!!sort.field)}function isSortArray(sort){return!!sort&&isArray(sort)}function isFacetMapping(f){return"row"in f||"column"in f}function toFieldDefBase(fieldDef){const {field,timeUnit,bin,aggregate}=fieldDef;return Object.assign({},timeUnit?{timeUnit}:{},bin?{bin}:{},aggregate?{aggregate}:{},{field})}function getBand($jscomp$destructuring$var22){var {channel,fieldDef,fieldDef2,markDef:mark,stack,config,isMidPoint}=$jscomp$destructuring$var22;if(isFieldOrDatumDef(fieldDef)&&void 0!==fieldDef.band)return fieldDef.band;
...
("binned"!==mark||(mark=encoding["x2"===channel?"x":"y"],isFieldDef(mark)&&isFieldDef(encoding[channel])&&isBinned(mark.bin)))?!0:!1}function initEncoding(encoding,mark,filled,config){return keys(encoding).reduce((normalizedEncoding,channel)=>{if(!CHANNEL_INDEX[channel])return warn(invalidEncodingChannel(channel)),normalizedEncoding;const channelDef=encoding[channel];"angle"!==channel||"arc"!==mark||encoding.theta||(warn("Arc marks uses theta channel rather than angle, replacing angle with theta."),
...
newChannelDef}return normalizedEncoding}function fieldDefs(encoding){const arr=[];for(const channel of keys(encoding))if(channelHasField(encoding,channel)){const channelDefArray=array$jscomp$0(encoding[channel]);for(const def of channelDefArray)isFieldDef(def)?arr.push(def):hasConditionalFieldDef(def)&&arr.push(def.condition)}return arr}function forEach(mapping,f,thisArg){if(mapping)for(const channel of keys(mapping)){const el=mapping[channel];if(isArray(el))for(const channelDef of el)f.call(thisArg,
channelDef,channel);else f.call(thisArg,el,channel)}}function reduce(mapping,f,init,thisArg){return mapping?keys(mapping).reduce((r,channel)=>{const map=mapping[channel];return isArray(map)?map.reduce((r1,channelDef)=>f.call(thisArg,r1,channelDef,channel),r):f.call(thisArg,r,map,channel)},init):init}function pathGroupingFields(mark,encoding){return keys(encoding).reduce((details,channel)=>{switch(channel){case "x":case "y":case "href":case "description":case "url":case "x2":case "y2":case "theta":case "theta2":case "radius":case "radius2":case "latitude":case "longitude":case "latitude2":case "longitude2":case "text":case "shape":case "angle":case "tooltip":return details;
...
(prop$jscomp$1=toProp)&&void 0!==prop$jscomp$1?prop$jscomp$1:prop$jscomp$3]);isEmpty(propConfig)||(prop$jscomp$2.style[null!==(prop$jscomp$0=toProp)&&void 0!==prop$jscomp$0?prop$jscomp$0:prop$jscomp$3]=propConfig);delete prop$jscomp$2[prop$jscomp$3]}for(const m of keys(compositeMarkRegistry))delete config[m];{markType=config;const {titleMarkConfig,subtitleMarkConfig,subtitle}=extractTitleConfig(markType.title);isEmpty(titleMarkConfig)||(markType.style["group-title"]=Object.assign({},markType.style["group-title"],
...
mapping[channel];isArray(channelDef)?out[channel]=channelDef.map(cd=>replaceRepeaterInChannelDef(cd,repeater)).filter(cd=>cd):(channelDef=replaceRepeaterInChannelDef(channelDef,repeater),void 0!==channelDef&&(out[channel]=channelDef))}return out}function mergeEncoding($jscomp$destructuring$var102){var {parentEncoding,encoding={},layer}=$jscomp$destructuring$var102;$jscomp$destructuring$var102={};if(parentEncoding){var channels=new Set([...keys(parentEncoding),...keys(encoding)]);for(const channel of channels){channels=
encoding[channel];const parentChannelDef=parentEncoding[channel];if(isFieldOrDatumDef(channels))channels=Object.assign({},parentChannelDef,channels),$jscomp$destructuring$var102[channel]=channels;else if(hasConditionalFieldOrDatumDef(channels))$jscomp$destructuring$var102[channel]=Object.assign({},channels,{condition:Object.assign({},parentChannelDef,channels.condition)});else if(channels||null===channels)$jscomp$destructuring$var102[channel]=channels;else if(layer||isValueDef(parentChannelDef)||
isSignalRef(parentChannelDef)||isFieldOrDatumDef(parentChannelDef)||isArray(parentChannelDef))$jscomp$destructuring$var102[channel]=parentChannelDef}}else $jscomp$destructuring$var102=encoding;return!$jscomp$destructuring$var102||isEmpty($jscomp$destructuring$var102)?void 0:$jscomp$destructuring$var102}function mergeProjection(opt){const {parentProjection:parentProjection$jscomp$0,projection:projection$jscomp$0}=opt;if(parentProjection$jscomp$0&&projection$jscomp$0){{const {parentProjection,projection}=
{parentProjection:parentProjection$jscomp$0,projection:projection$jscomp$0};opt="Layer's shared projection ".concat(stringify(parentProjection)," is overridden by a child projection ").concat(stringify(projection),".")}warn(opt)}return null!==projection$jscomp$0&&void 0!==projection$jscomp$0?projection$jscomp$0:parentProjection$jscomp$0}function normalize(spec,config){void 0===config&&(config=initConfig(spec.config));var JSCompiler_inline_result=config;JSCompiler_inline_result=void 0===JSCompiler_inline_result?
{}:JSCompiler_inline_result;JSCompiler_inline_result=normalizer.map(spec,{config:JSCompiler_inline_result});const {width:width$jscomp$0,height:height$jscomp$0}=spec;{let {width,height}={width:width$jscomp$0,height:height$jscomp$0,autosize:spec.autosize};spec=isUnitSpec(JSCompiler_inline_result)||"layer"in JSCompiler_inline_result;const autosizeDefault={};spec?"container"==width&&"container"==height?(autosizeDefault.type="fit",autosizeDefault.contains="padding"):"container"==width?(autosizeDefault.type=
"fit-x",autosizeDefault.contains="padding"):"container"==height&&(autosizeDefault.type="fit-y",autosizeDefault.contains="padding"):("container"==width&&(warn(containerSizeNonSingle("width")),width=void 0),"container"==height&&(warn(containerSizeNonSingle("height")),height=void 0));config=Object.assign({},{type:"pad"},autosizeDefault,config?_normalizeAutoSize(config.autosize):{},_normalizeAutoSize(JSCompiler_inline_result.autosize));"fit"!==config.type||spec||(warn('Autosize "fit" only works for single views and layered views.'),
...
data)&&!isGenerator(data)}function isGenerator(data){return data&&("sequence"in data||"sphere"in data||"graticule"in data)}function normalizeTransform(transform){return transform.map(t=>"filter"in t?{filter:normalizeLogicalComposition(t.filter,normalizePredicate)}:t)}function eventSelector(selector,source,marks){DEFAULT_SOURCE=source||"view";MARKS=marks||DEFAULT_MARKS;return parseMerge(selector.trim()).map(parseSelector)}function find(s,i,endChar,pushChar,popChar){const n=s.length;let count=0,c;for(;i<
n;++i)if(c=s[i],count||c!==endChar)popChar&&0<=popChar.indexOf(c)?--count:pushChar&&0<=pushChar.indexOf(c)&&++count;else break;return i}function parseMerge(s){const output=[],n=s.length;let start=0,i=0;for(;i<n;)i=find(s,i,",","[{","]}"),output.push(s.substring(start,i).trim()),start=++i;if(0===output.length)throw"Empty event selector: "+s;return output}function parseSelector(s){if("["===s[0]){var JSCompiler_temp=s;var n$jscomp$0=JSCompiler_temp.length;s=find(JSCompiler_temp,1,"]","[","]");if(s===
n$jscomp$0)throw"Empty between selector: "+JSCompiler_temp;n$jscomp$0=parseMerge(JSCompiler_temp.substring(1,s));if(2!==n$jscomp$0.length)throw"Between selector must have two elements: "+JSCompiler_temp;JSCompiler_temp=JSCompiler_temp.slice(s+1).trim();if("\x3e"!==JSCompiler_temp[0])throw"Expected '\x3e' after between selector: "+JSCompiler_temp;n$jscomp$0=n$jscomp$0.map(parseSelector);JSCompiler_temp=parseSelector(JSCompiler_temp.slice(1).trim());JSCompiler_temp.between?JSCompiler_temp={between:n$jscomp$0,
stream:JSCompiler_temp}:JSCompiler_temp.between=n$jscomp$0}else{{n$jscomp$0={source:DEFAULT_SOURCE};const source=[];let throttle=[0,0],markname=0,start=0,n=s.length,i=0,j;if("}"===s[n-1]){i=s.lastIndexOf("{");if(0<=i){try{throttle=parseThrottle(s.substring(i+1,n-1))}catch(e){throw"Invalid throttle specification: "+s;}s=s.slice(0,i).trim();n=s.length}else throw"Unmatched right brace: "+s;i=0}if(!n)throw s;"@"===s[0]&&(markname=++i);j=find(s,i,":");j<n&&(source.push(s.substring(start,j).trim()),start=
i=++j);i=find(s,i,"[");if(i===n)source.push(s.substring(start,n).trim());else if(source.push(s.substring(start,i).trim()),JSCompiler_temp=[],start=++i,start===n)throw"Unmatched left bracket: "+s;for(;i<n;){i=find(s,i,"]");if(i===n)throw"Unmatched left bracket: "+s;JSCompiler_temp.push(s.substring(start,i).trim());if(i<n-1&&"["!==s[++i])throw"Expected left bracket: "+s;start=++i}if(!(n=source.length)||ILLEGAL.test(source[n-1]))throw"Invalid event selector: "+s;1<n?(n$jscomp$0.type=source[1],markname?
n$jscomp$0.markname=source[0].slice(1):MARKS[source[0]]?n$jscomp$0.marktype=source[0]:n$jscomp$0.source=source[0]):n$jscomp$0.type=source[0];"!"===n$jscomp$0.type.slice(-1)&&(n$jscomp$0.consume=!0,n$jscomp$0.type=n$jscomp$0.type.slice(0,-1));null!=JSCompiler_temp&&(n$jscomp$0.filter=JSCompiler_temp);throttle[0]&&(n$jscomp$0.throttle=throttle[0]);throttle[1]&&(n$jscomp$0.debounce=throttle[1]);JSCompiler_temp=n$jscomp$0}}return JSCompiler_temp}function parseThrottle(s){const a=s.split(",");if(!s.length||
2<a.length)throw s;return a.map(_=>{_=+_;if(_!==_)throw s;return _})}function wrapCondition(model,channelDef,vgChannel,refFn){var condition=!!channelDef&&"condition"in channelDef&&channelDef.condition;channelDef=refFn(channelDef);return condition?(condition=array$jscomp$0(condition).map(c=>{const conditionValueRef=refFn(c);c=c.selection?parseSelectionPredicate(model,c.selection):expression$jscomp$0(model,c.test);return Object.assign({},{test:c},conditionValueRef)}),{[vgChannel]:[...condition,...void 0!==
channelDef?[channelDef]:[]]}):void 0!==channelDef?{[vgChannel]:channelDef}:{}}function text$jscomp$0(model,channel){channel=void 0===channel?"text":channel;return wrapCondition(model,model.encoding[channel],channel,cDef=>textRef(cDef,model.config))}function textRef(channelDef,config,expr){expr=void 0===expr?"datum":expr;if(channelDef){if(isValueDef(channelDef))return signalOrValueRef(channelDef.value);if(isFieldOrDatumDef(channelDef)){const {format,formatType}=getFormatMixins(channelDef);return formatSignalRef({fieldOrDatumDef:channelDef,
format,formatType,expr,config})}}}function tooltip(model,opt){opt=void 0===opt?{}:opt;const {encoding,markDef,config,stack}=model,channelDef=encoding.tooltip;if(isArray(channelDef))return{tooltip:tooltipRefForEncoding({tooltip:channelDef},stack,config,opt)};{const datum=opt.reactiveGeom?"datum.datum":"datum";return wrapCondition(model,channelDef,"tooltip",cDef=>{const tooltipRefFromChannelDef=textRef(cDef,config,datum);if(tooltipRefFromChannelDef)return tooltipRefFromChannelDef;if(null!==cDef){cDef=
...
CONTINUOUS_DOMAIN_INDEX&&(aggregator[channel]=!0));return aggregator},{});$jscomp$destructuring$var115=keys($jscomp$destructuring$var115);if(0<$jscomp$destructuring$var115.length){const op=invalid?"||":"\x26\x26";return $jscomp$destructuring$var115.map(field=>fieldInvalidPredicate(field,invalid)).join(" ".concat(op," "))}}function defined(model){const {config,markDef}=model;return getMarkPropOrConfig("invalid",markDef,config)&&(model=allFieldsInvalidPredicate$1(model,{channels:POSITION_SCALE_CHANNELS}))?
...
lex();var JSCompiler_object_inline_value_2635=new ASTNode("ArrayExpression");JSCompiler_object_inline_value_2635.elements=type;return type=JSCompiler_object_inline_value_2635}if(match("{")){type=[];JSCompiler_object_inline_value_2635={};var JSCompiler_object_inline_literal_2636=String;index$jscomp$0=lookahead.start;for(expect("{");!match("}");){index$jscomp$0=lookahead.start;var JSCompiler_object_inline_value_2637=lookahead;if(3===JSCompiler_object_inline_value_2637.type){var JSCompiler_object_inline_literal_2638=
parseObjectPropertyKey();expect(":");JSCompiler_object_inline_value_2637=parseConditionalExpression();JSCompiler_object_inline_value_2637=finishProperty("init",JSCompiler_object_inline_literal_2638,JSCompiler_object_inline_value_2637)}else 2===JSCompiler_object_inline_value_2637.type||7===JSCompiler_object_inline_value_2637.type?(throwUnexpected(JSCompiler_object_inline_value_2637),JSCompiler_object_inline_value_2637=void 0):(JSCompiler_object_inline_literal_2638=parseObjectPropertyKey(),expect(":"),
JSCompiler_object_inline_value_2637=parseConditionalExpression(),JSCompiler_object_inline_value_2637=finishProperty("init",JSCompiler_object_inline_literal_2638,JSCompiler_object_inline_value_2637));JSCompiler_object_inline_literal_2638="Identifier"===JSCompiler_object_inline_value_2637.key.type?JSCompiler_object_inline_value_2637.key.name:JSCompiler_object_inline_literal_2636(JSCompiler_object_inline_value_2637.key.value);JSCompiler_object_inline_literal_2638="$"+JSCompiler_object_inline_literal_2638;
Object.prototype.hasOwnProperty.call(JSCompiler_object_inline_value_2635,JSCompiler_object_inline_literal_2638)?throwError({},"Duplicate data property in object literal not allowed in strict mode"):JSCompiler_object_inline_value_2635[JSCompiler_object_inline_literal_2638]=!0;type.push(JSCompiler_object_inline_value_2637);match("}")||expect(",")}expect("}");JSCompiler_object_inline_value_2635=new ASTNode("ObjectExpression");JSCompiler_object_inline_value_2635.properties=type;return type=JSCompiler_object_inline_value_2635}type=
lookahead.type;index$jscomp$0=lookahead.start;if(3===type||legalKeywords[lookahead.value])JSCompiler_object_inline_value_2635=finishIdentifier(lex().value);else if(8===type||6===type)lookahead.octal&&throwError(lookahead,"Octal literals are not allowed in strict mode."),JSCompiler_object_inline_value_2635=finishLiteral(lex());else{if(4===type)throw Error("Disabled.");if(1===type)type=lex(),type.value="true"===type.value,JSCompiler_object_inline_value_2635=finishLiteral(type);else if(5===type)type=
lex(),type.value=null,JSCompiler_object_inline_value_2635=finishLiteral(type);else if(match("/")||match("/\x3d")){var value;lookahead=null;skipComment();type=index$jscomp$0;JSCompiler_object_inline_value_2635=source$jscomp$0[index$jscomp$0];assert("/"===JSCompiler_object_inline_value_2635,"Regular expression literal must start with a slash");JSCompiler_object_inline_literal_2636=source$jscomp$0[index$jscomp$0++];for(JSCompiler_object_inline_literal_2638=JSCompiler_object_inline_value_2637=!1;index$jscomp$0<
length$jscomp$0;)if(JSCompiler_object_inline_value_2635=source$jscomp$0[index$jscomp$0++],JSCompiler_object_inline_literal_2636+=JSCompiler_object_inline_value_2635,"\\"===JSCompiler_object_inline_value_2635)JSCompiler_object_inline_value_2635=source$jscomp$0[index$jscomp$0++],isLineTerminator(JSCompiler_object_inline_value_2635.charCodeAt(0))&&throwError({},"Invalid regular expression: missing /"),JSCompiler_object_inline_literal_2636+=JSCompiler_object_inline_value_2635;else if(isLineTerminator(JSCompiler_object_inline_value_2635.charCodeAt(0)))throwError({},
"Invalid regular expression: missing /");else if(JSCompiler_object_inline_value_2637)"]"===JSCompiler_object_inline_value_2635&&(JSCompiler_object_inline_value_2637=!1);else if("/"===JSCompiler_object_inline_value_2635){JSCompiler_object_inline_literal_2638=!0;break}else"["===JSCompiler_object_inline_value_2635&&(JSCompiler_object_inline_value_2637=!0);JSCompiler_object_inline_literal_2638||throwError({},"Invalid regular expression: missing /");JSCompiler_object_inline_value_2635=JSCompiler_object_inline_literal_2636.substr(1,
JSCompiler_object_inline_literal_2636.length-2);for(value=JSCompiler_object_inline_literal_2638="";index$jscomp$0<length$jscomp$0;){JSCompiler_object_inline_value_2637=source$jscomp$0[index$jscomp$0];if(!isIdentifierPart(JSCompiler_object_inline_value_2637.charCodeAt(0)))break;++index$jscomp$0;"\\"===JSCompiler_object_inline_value_2637&&index$jscomp$0<length$jscomp$0?throwError({},"Unexpected token %0","ILLEGAL"):(value+=JSCompiler_object_inline_value_2637,JSCompiler_object_inline_literal_2638+=JSCompiler_object_inline_value_2637)}0<=
value.search(/[^gimuy]/g)&&throwError({},"Invalid regular expression",value);JSCompiler_object_inline_value_2637=value;value=testRegExp(JSCompiler_object_inline_value_2635,JSCompiler_object_inline_value_2637);JSCompiler_object_inline_value_2635=finishLiteral({literal:JSCompiler_object_inline_literal_2636+JSCompiler_object_inline_literal_2638,value,regex:{pattern:JSCompiler_object_inline_value_2635,flags:JSCompiler_object_inline_value_2637},start:type,end:index$jscomp$0});peek()}else throwUnexpected(lex())}return JSCompiler_object_inline_value_2635}
...
parser(expression);const dependents=new Set;expression.visit(node=>{"MemberExpression"===node.type&&startsWithDatum(node)&&dependents.add(getName(node).slice(1).join("."))});return dependents}function parseUnitSelection(model,selDefs){const selCmpts={},selectionConfig=model.config.selection;for(const name of keys(null!==selDefs&&void 0!==selDefs?selDefs:{})){const selDef=duplicate(selDefs[name]);var $jscomp$destructuring$var130=Object.assign({},selectionConfig[selDef.type]);$jscomp$destructuring$var130=
...
config,channel,orient){return Object.assign.apply(null,[{},...configTypes.map(configType=>{if("axisOrient"===configType){configType="x"===channel?"bottom":"left";const orientConfig1=config["x"===channel?"axisBottom":"axisLeft"]||{},orientConfig2=config["x"===channel?"axisTop":"axisRight"]||{},props=new Set([...keys(orientConfig1),...keys(orientConfig2)]),conditionalOrientAxisConfig={};for(const prop of props.values())conditionalOrientAxisConfig[prop]={signal:"".concat(orient.signal,' \x3d\x3d\x3d "').concat(configType,
'" ? ').concat(signalOrStringValue(orientConfig1[prop])," : ").concat(signalOrStringValue(orientConfig2[prop]))};return conditionalOrientAxisConfig}return config[configType]})])}function getAxisConfigs(channel,scaleType,orient,config){var typeBasedConfigTypes="band"===scaleType?["axisDiscrete","axisBand"]:"point"===scaleType?["axisDiscrete","axisPoint"]:scaleType in QUANTITATIVE_SCALES_INDEX?["axisQuantitative"]:"time"===scaleType||"utc"===scaleType?["axisTemporal"]:[];const axisChannel="x"===channel?
"axisX":"axisY";scaleType=isSignalRef(orient)?"axisOrient":"axis"+titleCase(orient);typeBasedConfigTypes=[...typeBasedConfigTypes,...typeBasedConfigTypes.map(c=>axisChannel+c.substr(4))];scaleType=["axis",scaleType,axisChannel];return{vlOnlyAxisConfig:getAxisConfigFromConfigTypes(typeBasedConfigTypes,config,channel,orient),vgAxisConfig:getAxisConfigFromConfigTypes(scaleType,config,channel,orient),axisConfigStyle:getAxisConfigStyle([...scaleType,...typeBasedConfigTypes],config)}}function getAxisConfigStyle(axisConfigTypes,
config){const toMerge=[{}];for(const configType of axisConfigTypes){var _config$configType;if(axisConfigTypes=null===(_config$configType=config[configType])||void 0===_config$configType?void 0:_config$configType.style){axisConfigTypes=array$jscomp$0(axisConfigTypes);for(const s of axisConfigTypes)toMerge.push(config.style[s])}}return Object.assign.apply(null,toMerge)}function getAxisConfig(property,styleConfigIndex,style,axisConfigs){axisConfigs=void 0===axisConfigs?{}:axisConfigs;styleConfigIndex=
getStyleConfig(property,style,styleConfigIndex);if(void 0!==styleConfigIndex)return{configFrom:"style",configValue:styleConfigIndex};for(const configFrom of["vlOnlyAxisConfig","vgAxisConfig","axisConfigStyle"]){var _axisConfigs$configFr;if(void 0!==(null===(_axisConfigs$configFr=axisConfigs[configFrom])||void 0===_axisConfigs$configFr?void 0:_axisConfigs$configFr[property]))return{configFrom,configValue:axisConfigs[configFrom][property]}}return{}}function getLabelAngle(fieldOrDatumDef,axis,channel,
styleConfig,axisConfigs){const labelAngle=null===axis||void 0===axis?void 0:axis.labelAngle;if(void 0!==labelAngle)return isSignalRef(labelAngle)?labelAngle:normalizeAngle(labelAngle);({configValue:axis}=getAxisConfig("labelAngle",styleConfig,null===axis||void 0===axis?void 0:axis.style,axisConfigs));if(void 0!==axis)return normalizeAngle(axis);if("x"===channel&&contains(["nominal","ordinal"],fieldOrDatumDef.type)&&(!isFieldDef(fieldOrDatumDef)||!fieldOrDatumDef.timeUnit))return 270}function normalizeAngleExpr(angle){return"(((".concat(angle.signal,
" % 360) + 360) % 360)")}function defaultLabelBaseline(angle,orient,channel,alwaysIncludeMiddle){if(void 0!==angle)return"x"===channel?isSignalRef(angle)?(alwaysIncludeMiddle=normalizeAngleExpr(angle),orient=isSignalRef(orient)?"(".concat(orient.signal,' \x3d\x3d\x3d "top")'):"top"===orient,{signal:"(45 \x3c ".concat(alwaysIncludeMiddle," \x26\x26 ").concat(alwaysIncludeMiddle," \x3c 135) || (225 \x3c ").concat(alwaysIncludeMiddle," \x26\x26 ").concat(alwaysIncludeMiddle,' \x3c 315) ? "middle" :')+
"(".concat(alwaysIncludeMiddle," \x3c\x3d 45 || 315 \x3c\x3d ").concat(alwaysIncludeMiddle,") \x3d\x3d\x3d ").concat(orient,' ? "bottom" : "top"')}):45<angle&&135>angle||225<angle&&315>angle?"middle":isSignalRef(orient)?(alwaysIncludeMiddle=45>=angle||315<=angle?"\x3d\x3d\x3d":"!\x3d\x3d",{signal:"".concat(orient.signal," ").concat(alwaysIncludeMiddle,' "top" ? "bottom" : "top"')}):(45>=angle||315<=angle)===("top"===orient)?"bottom":"top":isSignalRef(angle)?(angle=normalizeAngleExpr(angle),orient=
isSignalRef(orient)?"(".concat(orient.signal,' \x3d\x3d\x3d "left")'):"left"===orient,alwaysIncludeMiddle=alwaysIncludeMiddle?'"middle"':"null",{signal:"".concat(angle," \x3c\x3d 45 || 315 \x3c\x3d ").concat(angle," || (135 \x3c\x3d ").concat(angle," \x26\x26 ").concat(angle," \x3c\x3d 225) ? ").concat(alwaysIncludeMiddle," : (45 \x3c\x3d ").concat(angle," \x26\x26 ").concat(angle," \x3c\x3d 135) \x3d\x3d\x3d ").concat(orient,' ? "top" : "bottom"')}):45>=angle||315<=angle||135<=angle&&225>=angle?
alwaysIncludeMiddle?"middle":null:isSignalRef(orient)?(alwaysIncludeMiddle=45<=angle&&135>=angle?"\x3d\x3d\x3d":"!\x3d\x3d",{signal:"".concat(orient.signal," ").concat(alwaysIncludeMiddle,' "left" ? "top" : "bottom"')}):(45<=angle&&135>=angle)===("left"===orient)?"top":"bottom"}function defaultLabelAlign(angle,orient,channel){if(void 0!==angle){var startAngle=(channel="x"===channel)?0:90,mainOrient=channel?"bottom":"left";return isSignalRef(angle)?(angle=normalizeAngleExpr(angle),orient=isSignalRef(orient)?
"(".concat(orient.signal,' \x3d\x3d\x3d "').concat(mainOrient,'")'):orient===mainOrient,{signal:"(".concat(startAngle?"("+angle+" + 90)":angle," % 180 \x3d\x3d\x3d 0) ? ").concat(channel?null:'"center"'," :")+"(".concat(startAngle," \x3c ").concat(angle," \x26\x26 ").concat(angle," \x3c ").concat(180+startAngle,") \x3d\x3d\x3d ").concat(orient,' ? "left" : "right"')}):0===(angle+startAngle)%180?channel?null:"center":isSignalRef(orient)?(channel=startAngle<angle&&angle<180+startAngle?"\x3d\x3d\x3d":
"!\x3d\x3d",orient="".concat(orient.signal," ").concat(channel,' "').concat(mainOrient,'"'),{signal:"".concat(orient,' ? "left" : "right"')}):(startAngle<angle&&angle<180+startAngle)===(orient===mainOrient)?"left":"right"}}function getFieldDefTitle(model,channel){var channel2="x"===channel?"x2":"y2";channel=model.fieldDef(channel);model=model.fieldDef(channel2);channel2=channel?channel.title:void 0;model=model?model.title:void 0;if(channel2&&model)return mergeTitle(channel2,model);if(channel2)return channel2;
if(model)return model;if(void 0!==channel2)return channel2;if(void 0!==model)return model}function sortArrayIndexField(fieldDef,channel,opt){return vgField(fieldDef,Object.assign({},{prefix:channel,suffix:"sort_index"},null!==opt&&void 0!==opt?opt:{}))}function getHeaderChannel(channel,orient){return contains(["top","bottom"],orient)?"column":contains(["left","right"],orient)?"row":"row"===channel?"row":"column"}function getHeaderProperty(prop,header,config,channel){return getFirstDefined((header||
...
roots)||modified,modified=runOptimizer(new MergeOutputs,roots)||modified);dataComponent.sources=roots;return modified}function parseUnitScaleDomain(model){const localScaleComponents=model.component.scales;for(const channel of keys(localScaleComponents)){var JSCompiler_inline_result=model;var channel$jscomp$0=channel,scaleType=JSCompiler_inline_result.getScaleComponent(channel$jscomp$0).get("type"),{encoding}=JSCompiler_inline_result;var JSCompiler_inline_result$jscomp$0=JSCompiler_inline_result.scaleDomain(channel$jscomp$0);
...
mergeValuesWithExplicit(JSCompiler_temp_const$jscomp$0,JSCompiler_temp,"type","scale",scaleTypeTieBreaker):(resolve.scale[channel]="independent",delete scaleTypeWithExplicitIndex[channel])}else scaleTypeWithExplicitIndex[channel]=JSCompiler_temp}}for(const channel of keys(scaleTypeWithExplicitIndex)){_resolve$scale$channe=model$jscomp$0.scaleName(channel,!0);scaleComponents[channel]=new ScaleComponent(_resolve$scale$channe,scaleTypeWithExplicitIndex[channel]);for(const child of model$jscomp$0.children)if(resolve=
...
transformNode.producedFields())&&void 0!==_transformNode$produc?_transformNode$produc:[]){var _transformNode$produc;ancestorParse.set(field,_ParseNode$makeExplic,!1)}}}_ParseNode$makeWithAn={};if(isUnitModel(model)&&model.component.selection)for(JSCompiler_inline_result of keys(model.component.selection)){_transformNode$produc=model.component.selection[JSCompiler_inline_result];for(var proj of _transformNode$produc.project.items)!proj.channel&&1<accessPathDepth(proj.field)&&(_ParseNode$makeWithAn[proj.field]=
...
axisEncodingPart}}void 0===axisEncodingPart||isEmpty(axisEncodingPart)||(e[part]={update:axisEncodingPart});return e},{});isEmpty(_axis$encoding)||axisComponent.set("encode",_axis$encoding,!!axis.encoding||void 0!==axis.labelAngle);return axisComponent}function orient(mark,encoding,specifiedOrient){switch(mark){case POINT:case CIRCLE:case SQUARE:case TEXT$1:case RECT:case IMAGE:return}const {x,y,x2,y2}=encoding;switch(mark){case BAR:if(isFieldDef(x)&&(isBinned(x.bin)||isFieldDef(y)&&y.aggregate&&
...
{align:"ignore",baseline:"ignore",color:"include",size:"include",orient:"ignore",theta:"ignore"}),pointPosition("x",model,{defaultPos:"mid"}),pointPosition("y",model,{defaultPos:"mid"}),nonPosition("size",model),nonPosition("angle",model),fixedShape?{shape:{value:fixedShape}}:nonPosition("shape",model))}function align(markDef,encoding,config){if(void 0===getMarkPropOrConfig("align",markDef,config))return"center"}function baseline(markDef,encoding,config){if(void 0===getMarkPropOrConfig("baseline",
...
JSCompiler_inline_result$jscomp$0=keys(model.component.selection).length,parent=model.parent;parent&&0===parentCount;)parentCount=keys(parent.component.selection).length,parent=parent.parent;JSCompiler_inline_result$jscomp$0=parentCount?{interactive:0<JSCompiler_inline_result$jscomp$0||!!model.encoding.tooltip}:null}else JSCompiler_inline_result$jscomp$0=null;parentCount=getMarkPropOrConfig("aria",markDef,config);parent=markCompiler[mark].postEncodingTransform?markCompiler[mark].postEncodingTransform(model):
...
tickDashOffset:{part:"ticks",vgProp:"strokeDashOffset"},tickOpacity:{part:"ticks",vgProp:"opacity"},tickSize:null,tickWidth:{part:"ticks",vgProp:"strokeWidth"}},AXIS_PARTS=["domain","grid","labels","ticks","title"],AXIS_PROPERTY_TYPE={grid:"grid",gridCap:"grid",gridColor:"grid",gridDash:"grid",gridDashOffset:"grid",gridOpacity:"grid",gridScale:"grid",gridWidth:"grid",orient:"main",bandPosition:"both",aria:"main",description:"main",domain:"main",domainCap:"main",domainColor:"main",domainDash:"main",
domainDashOffset:"main",domainOpacity:"main",domainWidth:"main",format:"main",formatType:"main",labelAlign:"main",labelAngle:"main",labelBaseline:"main",labelBound:"main",labelColor:"main",labelFlush:"main",labelFlushOffset:"main",labelFont:"main",labelFontSize:"main",labelFontStyle:"main",labelFontWeight:"main",labelLimit:"main",labelLineHeight:"main",labelOffset:"main",labelOpacity:"main",labelOverlap:"main",labelPadding:"main",labels:"main",labelSeparation:"main",maxExtent:"main",minExtent:"main",
offset:"both",position:"main",tickCap:"main",tickColor:"main",tickDash:"main",tickDashOffset:"main",tickMinStep:"main",tickOffset:"both",tickOpacity:"main",tickRound:"both",ticks:"main",tickSize:"main",tickWidth:"both",title:"main",titleAlign:"main",titleAnchor:"main",titleAngle:"main",titleBaseline:"main",titleColor:"main",titleFont:"main",titleFontSize:"main",titleFontStyle:"main",titleFontWeight:"main",titleLimit:"main",titleLineHeight:"main",titleOpacity:"main",titlePadding:"main",titleX:"main",
titleY:"main",encode:"both",scale:"both",tickBand:"both",tickCount:"both",tickExtra:"both",translate:"both",values:"both",zindex:"both"},COMMON_AXIS_PROPERTIES_INDEX={orient:1,aria:1,bandPosition:1,description:1,domain:1,domainCap:1,domainColor:1,domainDash:1,domainDashOffset:1,domainOpacity:1,domainWidth:1,format:1,formatType:1,grid:1,gridCap:1,gridColor:1,gridDash:1,gridDashOffset:1,gridOpacity:1,gridWidth:1,labelAlign:1,labelAngle:1,labelBaseline:1,labelBound:1,labelColor:1,labelFlush:1,labelFlushOffset:1,
...
titleFontWeight:1,titleLimit:1,titleLineHeight:1,titleOpacity:1,titlePadding:1,titleX:1,titleY:1,translate:1,values:1,zindex:1},AXIS_PROPERTIES_INDEX=Object.assign({},COMMON_AXIS_PROPERTIES_INDEX,{style:1,labelExpr:1,encoding:1}),AXIS_CONFIGS=keys({axis:1,axisBand:1,axisBottom:1,axisDiscrete:1,axisLeft:1,axisPoint:1,axisQuantitative:1,axisRight:1,axisTemporal:1,axisTop:1,axisX:1,axisXBand:1,axisXDiscrete:1,axisXPoint:1,axisXQuantitative:1,axisXTemporal:1,axisY:1,axisYBand:1,axisYDiscrete:1,axisYPoint:1,
axisYQuantitative:1,axisYTemporal:1}),AGGREGATE_OP_INDEX={argmax:1,argmin:1,average:1,count:1,distinct:1,product:1,max:1,mean:1,median:1,min:1,missing:1,q1:1,q3:1,ci0:1,ci1:1,stderr:1,stdev:1,stdevp:1,sum:1,valid:1,values:1,variance:1,variancep:1},MULTIDOMAIN_SORT_OP_INDEX={count:1,min:1,max:1},COUNTING_OPS=["count","valid","missing","distinct"],SUM_OPS=["count","sum","distinct","valid","missing"],SHARED_DOMAIN_OP_INDEX=toSet("mean average median q1 q3 min max".split(" ")),POLAR_POSITION_CHANNEL_INDEX=
{theta:1,theta2:1,radius:1,radius2:1},GEO_POSIITON_CHANNEL_INDEX={longitude:1,longitude2:1,latitude:1,latitude2:1},GEOPOSITION_CHANNELS=keys(GEO_POSIITON_CHANNEL_INDEX),UNIT_CHANNEL_INDEX=Object.assign({},{x:1,y:1,x2:1,y2:1},POLAR_POSITION_CHANNEL_INDEX,GEO_POSIITON_CHANNEL_INDEX,{color:1,fill:1,stroke:1,opacity:1,fillOpacity:1,strokeOpacity:1,strokeWidth:1,strokeDash:1,size:1,angle:1,shape:1,order:1,text:1,detail:1,key:1,tooltip:1,href:1,url:1,description:1}),FACET_CHANNEL_INDEX={row:1,column:1,
facet:1},FACET_CHANNELS=keys(FACET_CHANNEL_INDEX),CHANNEL_INDEX=Object.assign({},UNIT_CHANNEL_INDEX,FACET_CHANNEL_INDEX),CHANNELS=keys(CHANNEL_INDEX);var $jscomp$destructuring$var3=Object.assign({},CHANNEL_INDEX),$jscomp$destructuring$var4=(delete $jscomp$destructuring$var3.order,delete $jscomp$destructuring$var3.detail,delete $jscomp$destructuring$var3.tooltip,$jscomp$destructuring$var3),$jscomp$destructuring$var5=Object.assign({},$jscomp$destructuring$var4);const SINGLE_DEF_UNIT_CHANNEL_INDEX=(delete $jscomp$destructuring$var5.row,
delete $jscomp$destructuring$var5.column,delete $jscomp$destructuring$var5.facet,$jscomp$destructuring$var5),SECONDARY_RANGE_CHANNEL="x2 y2 latitude2 longitude2 theta2 radius2".split(" ");var $jscomp$destructuring$var7=Object.assign({},UNIT_CHANNEL_INDEX);const NONPOSITION_CHANNEL_INDEX=(delete $jscomp$destructuring$var7.x,delete $jscomp$destructuring$var7.y,delete $jscomp$destructuring$var7.x2,delete $jscomp$destructuring$var7.y2,delete $jscomp$destructuring$var7.latitude,delete $jscomp$destructuring$var7.longitude,
delete $jscomp$destructuring$var7.latitude2,delete $jscomp$destructuring$var7.longitude2,delete $jscomp$destructuring$var7.theta,delete $jscomp$destructuring$var7.theta2,delete $jscomp$destructuring$var7.radius,delete $jscomp$destructuring$var7.radius2,$jscomp$destructuring$var7),NONPOSITION_CHANNELS=keys(NONPOSITION_CHANNEL_INDEX),POSITION_SCALE_CHANNEL_INDEX={x:1,y:1},POSITION_SCALE_CHANNELS=keys(POSITION_SCALE_CHANNEL_INDEX),POLAR_POSITION_SCALE_CHANNEL_INDEX={theta:1,radius:1},POLAR_POSITION_SCALE_CHANNELS=
keys(POLAR_POSITION_SCALE_CHANNEL_INDEX);var $jscomp$destructuring$var9=Object.assign({},NONPOSITION_CHANNEL_INDEX);const NONPOSITION_SCALE_CHANNEL_INDEX=(delete $jscomp$destructuring$var9.text,delete $jscomp$destructuring$var9.tooltip,delete $jscomp$destructuring$var9.href,delete $jscomp$destructuring$var9.url,delete $jscomp$destructuring$var9.description,delete $jscomp$destructuring$var9.detail,delete $jscomp$destructuring$var9.key,delete $jscomp$destructuring$var9.order,$jscomp$destructuring$var9),
NONPOSITION_SCALE_CHANNELS=keys(NONPOSITION_SCALE_CHANNEL_INDEX),SCALE_CHANNEL_INDEX=Object.assign({},POSITION_SCALE_CHANNEL_INDEX,POLAR_POSITION_SCALE_CHANNEL_INDEX,NONPOSITION_SCALE_CHANNEL_INDEX),SCALE_CHANNELS=keys(SCALE_CHANNEL_INDEX),ALL_MARKS={arc:"always",area:"always",bar:"always",circle:"always",geoshape:"always",image:"always",line:"always",rule:"always",point:"always",rect:"always",square:"always",trail:"always",text:"always",tick:"always"};var $jscomp$destructuring$var11=Object.assign({},
ALL_MARKS);const ALL_MARKS_EXCEPT_GEOSHAPE=(delete $jscomp$destructuring$var11.geoshape,$jscomp$destructuring$var11),main=function(_$jscomp$0,method){let level=_$jscomp$0||0;return{level(_){return arguments.length?(level=+_,this):level},error(){1<=level&&log(method||"error","ERROR",arguments);return this},warn(){2<=level&&log(method||"warn","WARN",arguments);return this},info(){3<=level&&log(method||"log","INFO",arguments);return this},debug(){4<=level&&log(method||"log","DEBUG",arguments);return this}}}(2);
...
"year-month-date":"%b %d, %Y "},VG_MARK_CONFIGS=keys({aria:1,description:1,ariaRole:1,ariaRoleDescription:1,blend:1,opacity:1,fill:1,fillOpacity:1,stroke:1,strokeCap:1,strokeWidth:1,strokeOpacity:1,strokeDash:1,strokeDashOffset:1,strokeJoin:1,strokeOffset:1,strokeMiterLimit:1,startAngle:1,endAngle:1,padAngle:1,innerRadius:1,outerRadius:1,size:1,shape:1,interpolate:1,tension:1,orient:1,align:1,baseline:1,text:1,dir:1,dx:1,dy:1,ellipsis:1,limit:1,radius:1,theta:1,angle:1,font:1,fontSize:1,fontWeight:1,
fontStyle:1,lineBreak:1,lineHeight:1,cursor:1,href:1,tooltip:1,cornerRadius:1,cornerRadiusTopLeft:1,cornerRadiusTopRight:1,cornerRadiusBottomLeft:1,cornerRadiusBottomRight:1,aspect:1,width:1,height:1,url:1,smooth:1}),VG_MARK_INDEX={arc:1,area:1,group:1,image:1,line:1,path:1,rect:1,rule:1,shape:1,symbol:1,text:1,trail:1},VG_CORNERRADIUS_CHANNELS=["cornerRadius","cornerRadiusTopLeft","cornerRadiusTopRight","cornerRadiusBottomLeft","cornerRadiusBottomRight"],ScaleType={LINEAR:"linear",LOG:"log",POW:"pow",
SQRT:"sqrt",SYMLOG:"symlog",IDENTITY:"identity",SEQUENTIAL:"sequential",TIME:"time",UTC:"utc",QUANTILE:"quantile",QUANTIZE:"quantize",THRESHOLD:"threshold",BIN_ORDINAL:"bin-ordinal",ORDINAL:"ordinal",POINT:"point",BAND:"band"},SCALE_CATEGORY_INDEX={linear:"numeric",log:"numeric",pow:"numeric",sqrt:"numeric",symlog:"numeric",identity:"numeric",sequential:"numeric",time:"time",utc:"time",ordinal:"ordinal","bin-ordinal":"bin-ordinal",point:"ordinal-position",band:"ordinal-position",quantile:"discretizing",
quantize:"discretizing",threshold:"discretizing"},SCALE_PRECEDENCE_INDEX={linear:0,log:1,pow:1,sqrt:1,symlog:1,identity:1,sequential:1,time:0,utc:0,point:10,band:11,ordinal:0,"bin-ordinal":0,quantile:0,quantize:0,threshold:0},CONTINUOUS_TO_CONTINUOUS_SCALES="linear log pow sqrt symlog time utc".split(" "),CONTINUOUS_TO_CONTINUOUS_INDEX=toSet(CONTINUOUS_TO_CONTINUOUS_SCALES),QUANTITATIVE_SCALES_INDEX=toSet(["linear","log","pow","sqrt","symlog"]),CONTINUOUS_TO_DISCRETE_INDEX=toSet(["quantile","quantize",
"threshold"]),CONTINUOUS_DOMAIN_SCALES=CONTINUOUS_TO_CONTINUOUS_SCALES.concat(["quantile","quantize","threshold","sequential","identity"]),CONTINUOUS_DOMAIN_INDEX=toSet(CONTINUOUS_DOMAIN_SCALES),DISCRETE_DOMAIN_INDEX=toSet(["ordinal","bin-ordinal","point","band"]);var $jscomp$destructuring$var16=Object.assign({},{type:1,domain:1,domainMax:1,domainMin:1,domainMid:1,align:1,range:1,rangeMax:1,rangeMin:1,scheme:1,bins:1,reverse:1,round:1,clamp:1,nice:1,base:1,exponent:1,constant:1,interpolate:1,zero:1,
padding:1,paddingInner:1,paddingOuter:1});const NON_TYPE_DOMAIN_RANGE_VEGA_SCALE_PROPERTY_INDEX=(delete $jscomp$destructuring$var16.type,delete $jscomp$destructuring$var16.domain,delete $jscomp$destructuring$var16.range,delete $jscomp$destructuring$var16.rangeMax,delete $jscomp$destructuring$var16.rangeMin,delete $jscomp$destructuring$var16.scheme,$jscomp$destructuring$var16),NON_TYPE_DOMAIN_RANGE_VEGA_SCALE_PROPERTIES=keys(NON_TYPE_DOMAIN_RANGE_VEGA_SCALE_PROPERTY_INDEX),Mark={arc:"arc",area:"area",
bar:"bar",image:"image",line:"line",point:"point",rect:"rect",rule:"rule",text:"text",tick:"tick",trail:"trail",circle:"circle",square:"square",geoshape:"geoshape"},ARC=Mark.arc,AREA=Mark.area,BAR=Mark.bar,IMAGE=Mark.image,LINE=Mark.line,POINT=Mark.point,RECT=Mark.rect,RULE=Mark.rule,TEXT$1=Mark.text,TICK=Mark.tick,TRAIL=Mark.trail,CIRCLE=Mark.circle,SQUARE=Mark.square,GEOSHAPE=Mark.geoshape,PRIMITIVE_MARKS=keys(Mark);toSet(PRIMITIVE_MARKS);const FILL_STROKE_CONFIG="stroke strokeWidth strokeDash strokeDashOffset strokeOpacity strokeJoin strokeMiterLimit fill fillOpacity".split(" "),
VL_ONLY_MARK_CONFIG_PROPERTIES=keys({color:1,filled:1,invalid:1,order:1,radius2:1,theta2:1,timeUnitBand:1,timeUnitBandPosition:1}),MARK_CONFIGS=keys({mark:1,arc:1,area:1,bar:1,circle:1,image:1,line:1,point:1,rect:1,rule:1,square:1,text:1,tick:1,trail:1,geoshape:1}),BAR_CORNER_RADIUS_INDEX={horizontal:["cornerRadiusTopRight","cornerRadiusBottomRight"],vertical:["cornerRadiusTopLeft","cornerRadiusTopRight"]},SORT_BY_CHANNEL_INDEX={x:1,y:1,color:1,fill:1,stroke:1,strokeWidth:1,size:1,shape:1,fillOpacity:1,
strokeOpacity:1,opacity:1,text:1},defaultTitleFormatter=(fieldDef,config)=>{switch(config.fieldTitle){case "plain":return fieldDef.field;case "functional":{const {aggregate,bin,timeUnit,field}=fieldDef;isArgmaxDef(aggregate)?fieldDef="".concat(field," for argmax(").concat(aggregate.argmax,")"):isArgminDef(aggregate)?fieldDef="".concat(field," for argmin(").concat(aggregate.argmin,")"):(fieldDef=normalizeTimeUnit(timeUnit),fieldDef=(fieldDef=aggregate||(null===fieldDef||void 0===fieldDef?void 0:fieldDef.unit)||
(null===fieldDef||void 0===fieldDef?void 0:fieldDef.maxbins)&&"timeunit"||isBinning(bin)&&"bin")?fieldDef.toUpperCase()+"("+field+")":field)}return fieldDef;default:return verbalTitleFormatter(fieldDef,config)}};let titleFormatter=defaultTitleFormatter;const COMPATIBLE={compatible:!0};class CompositeMarkNormalizer{constructor(name,run){this.name=name;this.run=run}hasMatchingType(spec){return isUnitSpec(spec)?(spec=spec.mark,(isMarkDef(spec)?spec.type:spec)===this.name):!1}}const boxPlotNormalizer=
new CompositeMarkNormalizer("boxplot",normalizeBoxPlot),errorBarNormalizer=new CompositeMarkNormalizer("errorbar",normalizeErrorBar),errorBandNormalizer=new CompositeMarkNormalizer("errorband",normalizeErrorBand),compositeMarkRegistry={};add("boxplot",normalizeBoxPlot,["box","median","outliers","rule","ticks"]);add("errorbar",normalizeErrorBar,["ticks","rule"]);add("errorband",normalizeErrorBand,["band","borders"]);const VL_ONLY_LEGEND_CONFIG=["gradientHorizontalMaxLength","gradientHorizontalMinLength",
"gradientVerticalMaxLength","gradientVerticalMinLength","unselectedOpacity"],HEADER_TITLE_PROPERTIES_MAP={titleAlign:"align",titleAnchor:"anchor",titleAngle:"angle",titleBaseline:"baseline",titleColor:"color",titleFont:"font",titleFontSize:"fontSize",titleFontStyle:"fontStyle",titleFontWeight:"fontWeight",titleLimit:"limit",titleLineHeight:"lineHeight",titleOrient:"orient",titlePadding:"offset"},HEADER_LABEL_PROPERTIES_MAP={labelAlign:"align",labelAnchor:"anchor",labelAngle:"angle",labelBaseline:"baseline",
labelColor:"color",labelFont:"font",labelFontSize:"fontSize",labelFontStyle:"fontStyle",labelFontWeight:"fontWeight",labelLimit:"limit",labelLineHeight:"lineHeight",labelOrient:"orient",labelPadding:"offset"},HEADER_TITLE_PROPERTIES=keys(HEADER_TITLE_PROPERTIES_MAP),HEADER_LABEL_PROPERTIES=keys(HEADER_LABEL_PROPERTIES_MAP),HEADER_CONFIGS=keys({header:1,headerRow:1,headerColumn:1,headerFacet:1}),LEGEND_SCALE_CHANNELS="size shape fill stroke strokeDash strokeWidth opacity".split(" "),TOP_LEVEL_PROPERTIES=
["background","padding"],COMPOSITION_LAYOUT_PROPERTIES=keys({align:1,bounds:1,center:1,columns:1,spacing:1}),defaultConfig$1={background:"white",padding:5,timeFormat:"%b %d, %Y",countTitle:"Count of Records",view:{continuousWidth:200,continuousHeight:200,step:20},mark:{color:"#4c78a8",invalid:"filter",timeUnitBand:1},arc:{},area:{},bar:{binSpacing:1,continuousBandSize:5,timeUnitBandPosition:.5},circle:{},geoshape:{},image:{},line:{},point:{},rect:{binSpacing:0,continuousBandSize:5,timeUnitBandPosition:.5},
...
{p="visual"===range?p.channel:p.field;let sg=varName("".concat(name,"_").concat(p));for(let counter=1;signals.has(sg);counter++)sg=varName("".concat(name,"_").concat(p,"_").concat(counter));signals.add(sg);return{[range]:sg}};if(!selDef.fields&&!selDef.encodings){const cfg=model.config.selection[selDef.type];if(selDef.init)for(const init of array$jscomp$0(selDef.init))for(const key of keys(init))SINGLE_DEF_UNIT_CHANNEL_INDEX[key]?(selDef.encodings||(selDef.encodings=[])).push(key):"interval"===selDef.type?
(warn('Interval selections should be initialized using "x" and/or "y" keys.'),selDef.encodings=cfg.encodings):(selDef.fields||(selDef.fields=[])).push(key);else selDef.encodings=cfg.encodings,selDef.fields=cfg.fields}for(var field of null!==(_selDef$fields=selDef.fields)&&void 0!==_selDef$fields?_selDef$fields:[]){var _selDef$fields;_selDef$fields={type:"E",field};_selDef$fields.signals=Object.assign({},signalName(_selDef$fields,"data"));proj.items.push(_selDef$fields);proj.hasField[field]=_selDef$fields}for(const channel of null!==
(_selDef$encodings=selDef.encodings)&&void 0!==_selDef$encodings?_selDef$encodings:[]){var _selDef$encodings;(field=model.fieldDef(channel))?(_selDef$encodings=field.field,field.aggregate?warn(cannotProjectAggregate(channel,field.aggregate)):_selDef$encodings?(field.timeUnit&&(_selDef$encodings=model.vgField(channel),_selDef$fields={timeUnit:field.timeUnit,as:_selDef$encodings,field:field.field},_selCmpt$project[hash(_selDef$fields)]=_selDef$fields),parsed[_selDef$encodings]||(_selDef$fields="E",
"interval"===selCmpt.type?model.getScaleComponent(channel).get("type")in CONTINUOUS_DOMAIN_INDEX&&(_selDef$fields="R"):field.bin&&(_selDef$fields="R-RE"),field={field:_selDef$encodings,channel,type:_selDef$fields},field.signals=Object.assign({},signalName(field,"data"),signalName(field,"visual")),proj.items.push(parsed[_selDef$encodings]=field),proj.hasField[_selDef$encodings]=proj.hasChannel[channel]=parsed[_selDef$encodings])):warn(cannotProjectOnChannelWithoutField(channel))):warn(cannotProjectOnChannelWithoutField(channel))}selDef.init&&
(model=i=>proj.items.map(p=>void 0!==i[p.channel]?i[p.channel]:i[p.field]),"interval"===selDef.type?selCmpt.init=model(selDef.init):(selDef=array$jscomp$0(selDef.init),selCmpt.init=selDef.map(model)));isEmpty(_selCmpt$project)||(proj.timeUnit=new TimeUnitNode(null,_selCmpt$project))},signals:(model,selCmpt,allSignals)=>{const name=selCmpt.name+"_tuple_fields";return 0<allSignals.filter(s=>s.name===name).length?allSignals:allSignals.concat({name,value:selCmpt.project.items.map(proj=>{proj=Object.assign({},
proj);proj=(delete proj.signals,delete proj.hasLegend,proj);proj.field=replacePathInField(proj.field);return proj})})}},toggle,scaleBindings,legendBindings,{has:selCmpt=>"interval"===selCmpt.type&&selCmpt.translate,signals:(model,selCmpt,signals)=>{const name=selCmpt.name,hasScales=scaleBindings.has(selCmpt),anchor=name+"_translate_anchor",{x,y}=selCmpt.project.hasChannel;let events=eventSelector(selCmpt.translate,"scope");hasScales||(events=events.map(e=>(e.between[0].markname=name+"_brush",e)));
...
{},ancestorParse)}static makeWithAncestors(parent,explicit,implicit,ancestorParse){for(var field of keys(implicit)){const parsedAs=ancestorParse.getWithExplicit(field);void 0!==parsedAs.value&&(parsedAs.explicit||parsedAs.value===implicit[field]||"derived"===parsedAs.value||"flatten"===implicit[field]?delete implicit[field]:warn(differentParse(field,implicit[field],parsedAs.value)))}for(var field$jscomp$0 of keys(explicit))field=ancestorParse.get(field$jscomp$0),void 0!==field&&(field===explicit[field$jscomp$0]?
delete explicit[field$jscomp$0]:warn(differentParse(field$jscomp$0,explicit[field$jscomp$0],field)));explicit=new Split(explicit,implicit);ancestorParse.copyAll(explicit);implicit={};for(const key of keys(explicit.combine()))field$jscomp$0=explicit.get(key),null!==field$jscomp$0&&(implicit[key]=field$jscomp$0);return 0===keys(implicit).length||ancestorParse.parseNothing?null:new ParseNode(parent,implicit)}get parse(){return this._parse}merge(other){this._parse=Object.assign({},this._parse,other.parse);
other.remove()}assembleFormatParse(){const formatParse={};for(const field of keys(this._parse)){const p=this._parse[field];1===accessPathDepth(field)&&(formatParse[field]=p)}return formatParse}producedFields(){return new Set(keys(this._parse))}dependentFields(){return new Set(keys(this._parse))}assembleTransforms(onlyNested){onlyNested=void 0===onlyNested?!1:onlyNested;return keys(this._parse).filter(field=>onlyNested?1<accessPathDepth(field):!0).map(field=>{const expr=parseExpression$1(field,this._parse[field]);
...
params}dependentFields(){return new Set}producedFields(){}hash(){return"Graticule ".concat(hash(this.params))}assemble(){return Object.assign({},{type:"graticule"},!0===this.params?{}:this.params)}}class SequenceNode extends DataFlowNode{clone(){return new SequenceNode(null,this.params)}constructor(parent,params){super(parent);this.params=params}dependentFields(){return new Set}producedFields(){var _this$params$as;return new Set([null!==(_this$params$as=this.params.as)&&void 0!==_this$params$as?_this$params$as:
"data"])}hash(){return"Hash ".concat(hash(this.params))}assemble(){return Object.assign({},{type:"sequence"},this.params)}}class SourceNode extends DataFlowNode{constructor(data){var _data;super(null);_defineProperty(this,"_data",void 0);_defineProperty(this,"_name",void 0);_defineProperty(this,"_generator",void 0);data=null!==(_data=data)&&void 0!==_data?_data:{name:"source"};let format;isGenerator(data)||(format=data.format?Object.assign({},omit(data.format,["parse"])):{});if("values"in data)this._data=
{values:data.values};else if("url"in data)this._data={url:data.url},format.type||(_data=/(?:\.([^.]+))?$/.exec(data.url)[1],contains(["json","csv","tsv","dsv","topojson"],_data)||(_data="json"),format.type=_data);else if("sphere"in data)this._data={values:[{type:"Sphere"}]};else if(isNamedData(data)||isGenerator(data))this._data={};this._generator=isGenerator(data);data.name&&(this._name=data.name);format&&!isEmpty(format)&&(this._data.format=format)}dependentFields(){return new Set}producedFields(){}get data(){return this._data}hasName(){return!!this._name}get isGenerator(){return this._generator}get dataName(){return this._name}set dataName(name){this._name=
...
_modified.get(this);if(!JSCompiler_inline_result)throw new TypeError("attempted to get private field on non-instance");JSCompiler_inline_result=JSCompiler_inline_result.get?JSCompiler_inline_result.get.call(this):JSCompiler_inline_result.value;return JSCompiler_inline_result}}class BottomUpOptimizer extends Optimizer{getNodeDepths(node,depth,depths){depths.set(node,depth);for(const child of node.children)this.getNodeDepths(child,depth+1,depths);return depths}optimize(node){node=[...this.getNodeDepths(node,
0,new Map).entries()].sort((a,b)=>b[1]-a[1]);for(const tuple of node)this.run(tuple[0]);return this.modifiedFlag}}class TopDownOptimizer extends Optimizer{optimize(node){this.run(node);for(const child of node.children)this.optimize(child);return this.modifiedFlag}}class MergeIdenticalNodes extends TopDownOptimizer{mergeNodes(parent,nodes){const mergedNode=nodes.shift();for(const node of nodes)parent.removeChild(node),node.parent=mergedNode,node.remove()}run(node){const hashes=node.children.map(x=>
x.hash()),buckets={};for(let i=0;i<hashes.length;i++)void 0===buckets[hashes[i]]?buckets[hashes[i]]=[node.children[i]]:buckets[hashes[i]].push(node.children[i]);for(const k of keys(buckets))1<buckets[k].length&&(this.setModified(),this.mergeNodes(node,buckets[k]))}}class RemoveUnnecessaryIdentifierNodes extends TopDownOptimizer{constructor(model){super();_defineProperty(this,"requiresSelectionId",void 0);this.requiresSelectionId=model&&requiresSelectionId(model)}run(node){!(node instanceof IdentifierNode)||
this.requiresSelectionId&&(isDataSourceNode(node.parent)||node.parent instanceof AggregateNode||node.parent instanceof ParseNode)||(this.setModified(),node.remove())}}class RemoveDuplicateTimeUnits extends Optimizer{optimize(node){this.run(node,new Set);return this.modifiedFlag}run(node,timeUnitFields){let producedFields=new Set;node instanceof TimeUnitNode&&(producedFields=node.producedFields(),hasIntersection(producedFields,timeUnitFields)&&(this.setModified(),node.removeFormulas(timeUnitFields),
0===node.producedFields.length&&node.remove()));for(const child of node.children)this.run(child,new Set([...timeUnitFields,...producedFields]))}}class RemoveUnnecessaryOutputNodes extends TopDownOptimizer{constructor(){super()}run(node){node instanceof OutputNode&&!node.isRequired()&&(this.setModified(),node.remove())}}class MoveParseUp extends BottomUpOptimizer{run(node){if(!(isDataSourceNode(node)||1<node.numChildren()))for(const child of node.children)child instanceof ParseNode&&(node instanceof
ParseNode?(this.setModified(),node.merge(child)):fieldIntersection(node.producedFields(),child.dependentFields())||(this.setModified(),child.swapWithParent()))}}class MergeParse extends BottomUpOptimizer{run(node){const originalChildren=[...node.children];var parseChildren=node.children.filter(child=>child instanceof ParseNode);if(1<node.numChildren()&&1<=parseChildren.length){const commonParse={};var conflictingParse=new Set;for(const parseNode of parseChildren){parseChildren=parseNode.parse;for(const k of keys(parseChildren))k in
commonParse?commonParse[k]!==parseChildren[k]&&conflictingParse.add(k):commonParse[k]=parseChildren[k]}for(const field of conflictingParse)delete commonParse[field];if(!isEmpty(commonParse)){this.setModified();conflictingParse=new ParseNode(node,commonParse);for(const childNode of originalChildren){if(childNode instanceof ParseNode)for(const key of keys(commonParse))delete childNode.parse[key];node.removeChild(childNode);childNode.parent=conflictingParse;childNode instanceof ParseNode&&0===keys(childNode.parse).length&&
childNode.remove()}}}}}class RemoveUnusedSubtrees extends BottomUpOptimizer{run(node){node instanceof OutputNode||0<node.numChildren()||node instanceof FacetNode||node instanceof SourceNode||(this.setModified(),node.remove())}}class MergeTimeUnits extends BottomUpOptimizer{run(node){node=node.children.filter(x=>x instanceof TimeUnitNode);const combination=node.pop();for(const timeUnit of node)this.setModified(),combination.merge(timeUnit)}}class MergeAggregates extends BottomUpOptimizer{run(node){var aggChildren=
node.children.filter(child=>child instanceof AggregateNode);const groupedAggregates={};for(var agg of aggChildren)aggChildren=hash(agg.groupBy),aggChildren in groupedAggregates||(groupedAggregates[aggChildren]=[]),groupedAggregates[aggChildren].push(agg);for(const group of keys(groupedAggregates))if(agg=groupedAggregates[group],1<agg.length){aggChildren=agg.pop();for(const agg$jscomp$0 of agg)aggChildren.merge(agg$jscomp$0)&&(node.removeChild(agg$jscomp$0),agg$jscomp$0.parent=aggChildren,agg$jscomp$0.remove(),
this.setModified())}}}class MergeBins extends BottomUpOptimizer{constructor(model){super();this.model=model}run(node){var moveBinsUp=!(isDataSourceNode(node)||node instanceof FilterNode||node instanceof ParseNode||node instanceof IdentifierNode);const promotableBins=[],remainingBins=[];for(const child of node.children)child instanceof BinNode&&(moveBinsUp&&!fieldIntersection(node.producedFields(),child.dependentFields())?promotableBins.push(child):remainingBins.push(child));if(0<promotableBins.length){moveBinsUp=
promotableBins.pop();for(const bin of promotableBins)moveBinsUp.merge(bin,this.model.renameSignal.bind(this.model));this.setModified();node instanceof BinNode?node.merge(moveBinsUp,this.model.renameSignal.bind(this.model)):moveBinsUp.swapWithParent()}if(1<remainingBins.length){node=remainingBins.pop();for(const bin of remainingBins)node.merge(bin,this.model.renameSignal.bind(this.model));this.setModified()}}}class MergeOutputs extends BottomUpOptimizer{run(node){var children=[...node.children];if(some(children,
child=>child instanceof OutputNode)&&!(1>=node.numChildren())){var otherChildren=[];for(const child of children)if(child instanceof OutputNode){for(children=child;1===children.numChildren();){const [theChild]=children.children;if(theChild instanceof OutputNode)children=theChild;else break}otherChildren.push(...children.children);if(mainOutput)node.removeChild(child),child.parent=mainOutput.parent,mainOutput.parent.removeChild(mainOutput),mainOutput.parent=children,this.setModified();else var mainOutput=
children}else otherChildren.push(child);if(otherChildren.length){this.setModified();for(const child of otherChildren)child.parent.removeChild(child),child.parent=mainOutput}}}}class JoinAggregateTransformNode extends DataFlowNode{clone(){return new JoinAggregateTransformNode(null,duplicate(this.transform))}constructor(parent,transform){super(parent);this.transform=transform}addDimensions(fields){this.transform.groupby=unique(this.transform.groupby.concat(fields),d=>d)}dependentFields(){const out=
new Set;this.transform.groupby&&this.transform.groupby.forEach(out.add,out);this.transform.joinaggregate.map(w=>w.field).filter(f=>void 0!==f).forEach(out.add,out);return out}producedFields(){return new Set(this.transform.joinaggregate.map(this.getDefaultName))}getDefaultName(joinAggregateFieldDef){var _joinAggregateFieldDe;return null!==(_joinAggregateFieldDe=joinAggregateFieldDef.as)&&void 0!==_joinAggregateFieldDe?_joinAggregateFieldDe:vgField(joinAggregateFieldDef)}hash(){return"JoinAggregateTransform ".concat(hash(this.transform))}assemble(){const fields=
[],ops=[],as=[];for(var joinaggregate of this.transform.joinaggregate)ops.push(joinaggregate.op),as.push(this.getDefaultName(joinaggregate)),fields.push(void 0===joinaggregate.field?null:joinaggregate.field);joinaggregate=this.transform.groupby;return Object.assign({},{type:"joinaggregate",as,ops,fields},void 0!==joinaggregate?{groupby:joinaggregate}:{})}}class StackNode extends DataFlowNode{clone(){return new StackNode(null,duplicate(this._stack))}constructor(parent,stack){super(parent);_defineProperty(this,
"_stack",void 0);this._stack=stack}static makeFromTransform(parent,stackTransform){const {stack,groupby,as,offset="zero"}=stackTransform;var sortFields=[];const sortOrder=[];if(void 0!==stackTransform.sort)for(const sortField of stackTransform.sort)sortFields.push(sortField.field),sortOrder.push(getFirstDefined(sortField.order,"ascending"));sortFields={field:sortFields,order:sortOrder};stackTransform=isValidAsArray(as)?as:isString(as)?[as,as+"_end"]:[stackTransform.stack+"_start",stackTransform.stack+
"_end"];return new StackNode(parent,{stackField:stack,groupby,offset,sort:sortFields,facetby:[],as:stackTransform})}static makeFromEncoding(parent,model){var stackProperties=model.stack,{encoding}=model;if(!stackProperties)return null;const {groupbyChannel,fieldChannel,offset,impute}=stackProperties;let dimensionFieldDef;groupbyChannel&&(dimensionFieldDef=getFieldDef(encoding[groupbyChannel]));stackProperties=getStackByFields(model);encoding=model.encoding.order;encoding=isArray(encoding)||isFieldDef(encoding)?
sortParams(encoding):stackProperties.reduce((s,field)=>{s.field.push(field);s.order.push("y"===fieldChannel?"descending":"ascending");return s},{field:[],order:[]});return new StackNode(parent,{dimensionFieldDef,stackField:model.vgField(fieldChannel),facetby:[],stackby:stackProperties,sort:encoding,offset,impute,as:[model.vgField(fieldChannel,{suffix:"start",forAs:!0}),model.vgField(fieldChannel,{suffix:"end",forAs:!0})]})}get stack(){return this._stack}addDimensions(fields){this._stack.facetby.push(...fields)}dependentFields(){const out=
...
typeWithExplicit){super({},{name});_defineProperty(this,"merged",!1);this.setWithExplicit("type",typeWithExplicit)}domainDefinitelyIncludesZero(){return!1!==this.get("zero")?!0:some(this.get("domains"),d=>isArray(d)&&2===d.length&&0>=d[0]&&0<=d[1])}}const RANGE_PROPERTIES=["range","scheme"],scaleRules={bins:$jscomp$destructuring$var178=>{var {model,fieldOrDatumDef}=$jscomp$destructuring$var178;return isFieldDef(fieldOrDatumDef)?bins(model,fieldOrDatumDef):void 0},interpolate:$jscomp$destructuring$var179=>
...
parentGivenName,config){super(spec,"facet",parent,parentGivenName,config,spec.resolve);_defineProperty(this,"facet",void 0);_defineProperty(this,"child",void 0);_defineProperty(this,"children",void 0);this.child=buildModel(spec.spec,this,this.getName("child"),void 0,config);this.children=[this.child];this.facet=this.initFacet(spec.facet)}initFacet(facet){if(!isFacetMapping(facet))return{facet:this.initFacetFieldDef(facet,"facet")};var channels=keys(facet);const normalizedFacet={};for(const channel of channels){if(!contains(["row",
...
{};for(const child of this.children){child.parseSelections();for(const key of keys(child.component.selection))this.component.selection[key]=child.component.selection[key]}}parseMarkGroup(){for(const child of this.children)child.parseMarkGroup()}parseAxesAndHeaders(){{const {axes,resolve}=this.component,axisCount={top:0,bottom:0,right:0,left:0};for(var child$jscomp$0 of this.children){child$jscomp$0.parseAxesAndHeaders();for(const channel of keys(child$jscomp$0.component.axes))resolve.axis[channel]=
...
accessPathWithDatum;exports.compile=function(inputSpec,opt){opt=void 0===opt?{}:opt;opt.logger&&(current=opt.logger);opt.fieldTitle&&(titleFormatter=opt.fieldTitle);try{const config=initConfig(mergeConfig(opt.config,inputSpec.config)),spec=normalize(inputSpec,config),model=buildModel(spec,null,"",void 0,config);model.parse();{var data=model.component.data,model$jscomp$0=model;checkLinks(data.sources);let firstPassCounter=0,secondPassCounter=0;for(var i=0;5>i&&optimizationDataflowHelper(data,model$jscomp$0,
!0);i++)firstPassCounter++;data.sources.map(moveFacetDown);for(i=0;5>i&&optimizationDataflowHelper(data,model$jscomp$0,!1);i++)secondPassCounter++;checkLinks(data.sources);5===Math.max(firstPassCounter,secondPassCounter)&&warn("Maximum optimization runs(".concat(5,") reached."))}{var autosize=spec.autosize;data=config;model$jscomp$0=model;const width=model$jscomp$0.component.layoutSize.get("width"),height=model$jscomp$0.component.layoutSize.get("height");void 0===autosize?(autosize={type:"pad"},model$jscomp$0.hasAxisOrientSignalRef()&&
(autosize.resize=!0)):isString(autosize)&&(autosize={type:autosize});var JSCompiler_temp;if(JSCompiler_temp=width&&height){var autoSizeType=autosize.type;JSCompiler_temp="fit"===autoSizeType||"fit-x"===autoSizeType||"fit-y"===autoSizeType}if(JSCompiler_temp)if("step"===width&&"step"===height)warn(droppingFit()),autosize.type="pad";else if("step"===width||"step"===height){autoSizeType="step"===width?"width":"height";warn(droppingFit(getPositionScaleChannel(autoSizeType)));JSCompiler_temp=autosize;
var JSCompiler_inline_result=(autoSizeType="width"===autoSizeType?"height":"width","fit-".concat(getPositionScaleChannel(autoSizeType)));JSCompiler_temp.type=JSCompiler_inline_result}var JSCompiler_inline_result$jscomp$0=Object.assign({},1===keys(autosize).length&&autosize.type?"pad"===autosize.type?{}:{autosize:autosize.type}:{autosize},extractTopLevelProperties(data,!1),extractTopLevelProperties(inputSpec,!0))}return{spec:assembleTopLevelModel(model,JSCompiler_inline_result$jscomp$0,inputSpec.datasets,
inputSpec.usermeta),normalized:spec}}finally{opt.logger&&(current=main),opt.fieldTitle&&(titleFormatter=defaultTitleFormatter)}};exports.contains=contains;exports.deepEqual=deepEqual;exports.deleteNestedProperty=deleteNestedProperty;exports.duplicate=duplicate;exports.entries=entries;exports.every=every;exports.fieldIntersection=fieldIntersection;exports.flatAccessWithDatum=flatAccessWithDatum;exports.getFirstDefined=getFirstDefined;exports.hasIntersection=hasIntersection;exports.hash=hash;exports.internalField=
internalField;exports.isBoolean=isBoolean$1;exports.isEmpty=isEmpty;exports.isEqual=function(dict,other){const dictKeys=keys(dict),otherKeys=keys(other);if(dictKeys.length!==otherKeys.length)return!1;for(const key of dictKeys)if(dict[key]!==other[key])return!1;return!0};exports.isInternalField=isInternalField;exports.isNullOrFalse=isNullOrFalse;exports.isNumeric=isNumeric;exports.keys=keys;exports.logicalExpr=logicalExpr;exports.mergeDeep=mergeDeep;exports.normalize=normalize;exports.normalizeAngle=
normalizeAngle;exports.omit=omit;exports.pick=pick;exports.prefixGenerator=prefixGenerator;exports.removePathFromField=removePathFromField;exports.replaceAll=replaceAll;exports.replacePathInField=replacePathInField;exports.resetIdCounter=function(){idCounter=42};exports.setEqual=setEqual;exports.some=some;exports.stringify=stringify;exports.titleCase=titleCase;exports.unique=unique;exports.uniqueId=uniqueId;exports.vals=vals;exports.varName=varName;exports.version="4.17.0";Object.defineProperty(exports,
"__esModule",{value:!0})})}
