...
(_core.log.deprecated("thresholdValue","threshold")(),threshold=opts.thresholdValue);opts=0>x;var isRightBoundary=x>=width-1,isBottomBoundary=0>y,isTopBoundary=y>=height-1;height=opts||isRightBoundary||isBottomBoundary||isTopBoundary;if(opts||isTopBoundary)var JSCompiler_object_inline_top_4413=0;else{var JSCompiler_object_inline_top_4409=cellWeights[(y+1)*width+x];JSCompiler_object_inline_top_4413=getVertexCode(JSCompiler_object_inline_top_4409,threshold)}if(isRightBoundary||isTopBoundary)isTopBoundary=
0;else{var JSCompiler_object_inline_topRight_4410=cellWeights[(y+1)*width+x+1];isTopBoundary=getVertexCode(JSCompiler_object_inline_topRight_4410,threshold)}if(isRightBoundary||isBottomBoundary)isRightBoundary=0;else{var JSCompiler_object_inline_right_4411=cellWeights[y*width+x+1];isRightBoundary=getVertexCode(JSCompiler_object_inline_right_4411,threshold)}if(opts||isBottomBoundary)x=0;else{var JSCompiler_object_inline_current_4412=cellWeights[y*width+x];x=getVertexCode(JSCompiler_object_inline_current_4412,
threshold)}cellWeights=JSCompiler_object_inline_top_4413;y=isTopBoundary;width=isRightBoundary;opts=x;x=-1;Number.isFinite(threshold)&&(x=cellWeights<<3|y<<2|width<<1|opts);Array.isArray(threshold)&&(x=cellWeights<<6|y<<4|width<<2|opts);cellWeights=0;height||(cellWeights=getVertexCode((JSCompiler_object_inline_top_4409+JSCompiler_object_inline_topRight_4410+JSCompiler_object_inline_right_4411+JSCompiler_object_inline_current_4412)/4,threshold));return{code:x,meanCode:cellWeights}};exports.getVertices=
